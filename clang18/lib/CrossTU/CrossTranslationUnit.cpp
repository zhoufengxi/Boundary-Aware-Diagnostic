...
//
//===----------------------------------------------------------------------===//
// 模板特化修改 不对case v2 奇怪断链 后crash(删掉了许多旧的日志) 后crash Segmentation fault2 + 重复LOAD优化, 又crash修复
...
static bool CSAIsArkCompilerGraphFactoryTemplate(const FunctionDecl *FD) {
  ...
  return Name.starts_with("CreateInst") || Name.starts_with("FindOrCreateInst") ||
         Name.starts_with("GetOrCreateInst");
}
...
static CSACTUTemplateAction CSAGetCTUTemplateAction(const FunctionDecl *FD) {
  ...
  // 1. ArkCompiler Graph/Inst 工厂模板：高频、参数包、大量实例化，
  // 对 NewDelete/异步所有权链通常不是核心目标，直接跳过。
  ...
  // 2. 参数包模板优先跳过。这个判断比扫描 body 便宜。
  // ir_builder.cpp 中大量 CreateInstXXX 都会命中这里。
  if (CSAFunctionTemplateHasParameterPack(FD))
    return CSACTUTemplateAction::SkipAndStop;

  // 3. 标准库模板不能一刀切。
  //    - std::for_each 这种驱动用户 lambda 的模板允许实例化；
  //    - __ostream_insert / _M_insert / tuple::operator= 这类不驱动用户
  //      callable 的模板直接跳过。
  ...
  // 4. range-for 模板实例化曾在 Sema::BuildCXXForRangeStmt crash。
  if (CSAStmtContainsCXXForRange(Body))
    return CSACTUTemplateAction::SkipAndStop;

  // 5. 大 body 跳过，避免 CTU fallback 把复杂工程模板拉爆。
  unsigned NodeCount = CSAStmtNodeCount(Body);
  if (NodeCount > 300)
    return CSACTUTemplateAction::SkipAndStop;

  // 6. 简单项目模板，例如小 tfunc case，允许递归实例化。
  return CSACTUTemplateAction::InstantiateRecursive;
}
...
CrossTranslationUnitContext::getCrossTUDefinition(const FunctionDecl *FD,
  ...
  const std::optional<std::string> LookupName = getLookupName(FD);
  if (!LookupName)
    return Result.takeError();
  ...
  // Early terminal skip:
  // 如果当前 FD 本身就是一个无 body 的函数模板特化，并且已经被策略判定为
  // SkipAndStop，则不要再进入 ASTStorage 全量扫描。这样可以避免
  // __ostream_insert / _M_insert 在多个 ASTUnit 中反复命中、反复打印。
      ...
      auto IndexDeclContext = [&](const DeclContext *DC,
        ...
        for (const Decl *D : DC->decls()) {
          ...
          auto AddDecl = [&](const FunctionDecl *Candidate) {
            if (!Candidate)
              return;
            std::optional<std::string> CandidateName =
  ...
  for (const auto &Entry : ASTStorage.getFileASTUnitMap()) {
    ...
    if (const FunctionDecl *FallbackFD =
      ...
      if (FallbackFD->getTemplatedKind() ==
        ...
        if (Action == CSACTUTemplateAction::InstantiateRecursive) {
          // 不在 CTU runtime definition 查询中递归清空 pending instantiations。
          // 真实工程里 Recursive=true / AtEndOfTU=true 容易扩散到 nlohmann/json、
          // libc++、系统头里的复杂模板实例化。
          Recursive = false;
          AtEndOfTU = false;
        }
      ...
      if (!ImportedFD->hasBody(ImportedDef) || !ImportedDef ||
          !ImportedDef->getBody()) {
        FallbackDeclFailureGeneration[CanonicalFallbackFD] =
            FallbackGeneration;
        continue;
    ...
    if (const FunctionDecl *Pat = FD->getTemplateInstantiationPattern()) {
      ...
      if (std::optional<std::string> PatternLookup = getLookupName(Pat)) {
