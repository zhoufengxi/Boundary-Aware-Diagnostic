...
//
//===----------------------------------------------------------------------===//
// 模板特化修改 不对case v2 + Segmentation fault (core dumped)查看
// 奇怪断链 后crash Segmentation fault2 又crash修复
...
static bool CSA_LAMBDA_DBG() {
  return std::getenv("CSA_LAMBDA_DBG") != nullptr;
}

static bool CSA_CTU_DBG() {
  return std::getenv("CSA_CTU_DBG") != nullptr;
}
...
static bool shouldCountFunctionImportReentry(const Decl *D) {
  ...
  // 不统计纯声明，否则 memcpy/free/operator delete/std deduction guide
  // 这类无 body 声明会产生噪声。
...
ExpectedDecl ASTNodeImporter::VisitFunctionDecl(FunctionDecl *D) {
  ...
  // CSA CTU fix: lambda that imports only the body into an existing
  // bodyless FunctionTemplateSpecialization.  Instead of creating new
  // ParmVarDecls, the existing To-params are mapped from the From-params so
  // that DeclRefExprs inside the imported body resolve correctly.
  ...
  // CSA CTU fix: guard setPreviousDecl with ToFunction != Recent — if we are
  // only merging a body, ToFunction may already be the most recent decl and
  // reconnecting would corrupt the (possibly lazy) redecl chain.
  if (FoundByLookup) {
...
Expected<DeclarationName> ASTImporter::HandleNameConflict(DeclarationName Name,
  ...
  if (ODRHandling == ODRHandlingType::Conservative) {
    ...
    // CSA CTU fix: if ALL conflicting decls are in system headers, suppress
    // the conflict and allow the new Decl to be created with the same name.
    bool AllSystem = true;
