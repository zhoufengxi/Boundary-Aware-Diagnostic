...
void PathDiagnosticConsumer::HandlePathDiagnostic(
  ...
  if (PathDiagnostic *orig = Diags.FindNodeOrInsertPos(profile, InsertPos)) {
    if (custom_analyzer_fixes::isReportPathFixEnabled()) {
      // Prefer the PathDiagnostic whose path carries a CSA allocation origin.
      // If both (or neither) carry an origin, fall back to the shorter-path
      // policy so behavior stays consistent with the deterministic call order.
      const bool OrigHasOrigin = hasCSAAllocationOrigin(*orig);
      const bool NewHasOrigin = hasCSAAllocationOrigin(*D);

      if (OrigHasOrigin && !NewHasOrigin)
        return;
      ...
      if (!OrigHasOrigin && NewHasOrigin) {
        ...
        // Note, the enclosing routine is called in deterministic order, so
        // the results will be consistent between runs (no reason to break
        // ties if the size is the same).
      ...
      // results will be consistent between runs (no reason to break ties if
