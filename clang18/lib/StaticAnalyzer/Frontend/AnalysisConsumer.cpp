...
getCustomAnalyzerFixState(const AnalyzerOptions &Opts) {
...
static bool isNewDeleteRelevancePruningEnabled(const AnalyzerOptions &Opts) {
...
public:
  ...
  bool VisitCallExpr(const CallExpr *CE) {
    ...
    // Pointer-returning calls are sensitive for NewDeleteLeaks; do not skip
    // the caller.
    ...
    if (!Callee) {
      // Function pointer, virtual call, indirect call: conservatively relevant.
    ...
    // Direct call without a body may allocate/free/store pointers in an
    // external TU or library. First version is conservative to avoid false
    // negatives.
    if (!Callee->hasBody())
      Summary.HasUnknownCall = true;
...
static std::string
getExplicitNewDeleteEntryConfigPath(const AnalyzerOptions &Opts) {
...
static bool
isExplicitNewDeleteEntryFilterEnabled(const AnalyzerOptions &Opts) {
  return !getExplicitNewDeleteEntryConfigPath(Opts).empty();
}

static void loadExplicitNewDeleteEntryConfig(const AnalyzerOptions &Opts,
...
getExplicitNewDeleteEntryConfig(const AnalyzerOptions &Opts) {
...
static void dumpExplicitNewDeleteEntryDecision(StringRef Tag, const Decl *D,
                                               bool Allowed,
                                               StringRef Reason) {
  if (CSA_CTU_VERBOSE()) {
    ...
    if (std::optional<std::string> LookupName =
            getDeclLookupNameForEntryFilter(D)) {
...
void AnalysisConsumer::HandleDeclsCallGraph(const unsigned LocalTUDeclsSize) {
  ...
  for (auto &N : RPOT) {
    ...
    // In explicit-entry mode, the allowlist itself controls which functions
    // are analyzed as top-level roots. Do not use the global Visited
    // heuristic here, because Visited contains callees inlined from previous
    // roots. Using it may accidentally suppress configured entries or affect
    // later inlining decisions.
    ...
    // Explicit entry filter has higher priority than memory-relevance
    // pruning. If a config file is provided, only listed functions are
    // analyzed as top-level entries. Listed functions must not be pruned
    // again by the heuristic relevance filter.
    if (ExplicitEntryMode) {
    ...
    // must remain available for normal inline/CTU analysis when reached from
    // an allowed entry.
