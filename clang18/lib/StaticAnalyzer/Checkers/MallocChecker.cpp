...
//
//   * UnmodeledAllocationEscapeChecker
//       Reports live malloc/new/new[] allocations that escape through calls
//       whose C or C++ definition is unavailable.
//
...
static bool stmtTreeMentionsSymbol(const Stmt *S, ProgramStateRef State,
...
static bool stmtTreeCanReachSymbol(const Stmt *S, ProgramStateRef State,
...
REGISTER_MAP_WITH_PROGRAMSTATE(ConfiguredDeallocator, SymbolRef,
                               const FunctionDecl *)
REGISTER_SET_WITH_PROGRAMSTATE(PendingUnmodeledAllocationEscapes, SymbolRef)
...
public:
  ...
  enum CheckKind {
    ...
    CK_UnmodeledAllocationEscapeChecker,
    CK_NumCheckKinds
  };
...
private:
  ...
  mutable std::unique_ptr<BugType> BT_UnmodeledAllocationEscape;
...
#define CHECK_FN(NAME)                                                         \
  ...
  /// Emit and clear allocation-escape diagnostics recorded while the current
  /// call invalidated its arguments.
  bool reportUnmodeledAllocationEscapes(CheckerContext &C) const;
...
bool MallocChecker::reportUnmodeledAllocationEscapes(
    CheckerContext &C) const {
  if (!ChecksEnabled[CK_UnmodeledAllocationEscapeChecker])
    return false;

  ProgramStateRef State = C.getState();
  auto Pending = State->get<PendingUnmodeledAllocationEscapes>();
  if (Pending.isEmpty())
    return false;

  SmallVector<SymbolRef, 4> Symbols;
  for (SymbolRef Sym : Pending)
    Symbols.push_back(Sym);
  for (SymbolRef Sym : Symbols)
    State = State->remove<PendingUnmodeledAllocationEscapes>(Sym);

  static CheckerProgramPointTag Tag("MallocChecker",
                                    "UnmodeledAllocationEscape");
  ExplodedNode *N = C.generateNonFatalErrorNode(State, &Tag);
  if (!N) {
    C.addTransition(State);
    return true;
  }

  if (!BT_UnmodeledAllocationEscape)
    BT_UnmodeledAllocationEscape.reset(new BugType(
        CheckNames[CK_UnmodeledAllocationEscapeChecker],
        "Unmodeled allocation escape", categories::MemoryError));

  constexpr llvm::StringLiteral Message =
      "Allocated memory escapes through a call with no available definition; "
      "it may be released or retained";
  ...
  for (SymbolRef Sym : Symbols) {
    ...
    if (const Stmt *AllocationStmt = AllocNode->getStmtForDiagnostics())
      UniqueingLocation = PathDiagnosticLocation::createBegin(
          AllocationStmt, C.getSourceManager(),
          AllocNode->getLocationContext());

    auto Report = std::make_unique<PathSensitiveBugReport>(
        *BT_UnmodeledAllocationEscape, Message, N, UniqueingLocation,
        AllocNode->getLocationContext()->getDecl());
    Report->markInteresting(Sym);
    Report->addVisitor<MallocBugVisitor>(Sym);
...
void MallocChecker::checkPostCall(const CallEvent &Call,
                                  CheckerContext &C) const {
  if (reportUnmodeledAllocationEscapes(C))
    return;
                                     ...
                                     CheckerContext &C) const
{
  ...
  for (auto [Sym, State] : RS) {
    if (SymReaper.isDead(Sym)) {
      ...
      state = state->remove<ConfiguredDeallocator>(Sym);
      RS = F.remove(RS, Sym);
    }
  }
...
void MallocChecker::checkPreCall(const CallEvent &Call,
  ...
  if (const auto *DC = dyn_cast<CXXDeallocatorCall>(&Call)) {
    ...
    const NoteTag *ConfiguredCallbackReleaseNote = nullptr;
    if (State && State->get<ActiveConfiguredCallbackSequence>()) {
      SymbolRef ReleasedSym = nullptr;
      if (const MemRegion *R = DeleteArgVal.getAsRegion()) {
        R = R->StripCasts();
        if (const auto *SR = dyn_cast<SymbolicRegion>(R->getBaseRegion()))
          ReleasedSym = SR->getSymbol();
      }

      const RefState *Before =
          ReleasedSym ? C.getState()->get<RegionState>(ReleasedSym) : nullptr;
      const RefState *After =
          ReleasedSym ? State->get<RegionState>(ReleasedSym) : nullptr;

      if (After && After->isReleased() && (!Before || !Before->isReleased())) {
        ConfiguredCallbackReleaseNote =
            C.getNoteTag([ReleasedSym](PathSensitiveBugReport &BR) {
              const bool Interesting =
                  ReleasedSym && BR.isInteresting(ReleasedSym);
              if (!Interesting)
                return std::string();
              return std::string("Memory is released");
            });
      }
    }

    C.addTransition(State, ConfiguredCallbackReleaseNote);
    return;
  }
  ...
  const auto *CalledFunction = dyn_cast_or_null<FunctionDecl>(Call.getDecl());
  for (unsigned I = 0, E = Call.getNumArgs(); I != E; ++I) {
    ...
    if (isa<Loc>(ArgSVal)) {
      ...
      if (!Sym)
        continue;
      if (allocation_state::wasReleasedBy(C.getState(), Sym, CalledFunction))
        continue;
...
static bool isUnmodeledEscapeAllocationFamily(const RefState *RS) {
  switch (RS->getAllocationFamily()) {
  case AF_Malloc:
  case AF_CXXNew:
  case AF_CXXNewArray:
    return true;
...
static bool callHasNoAvailableCOrCXXDefinition(const CallEvent *Call) {
  if (!Call || !Call->getOriginExpr() || isa<ObjCMethodCall>(Call))
    return false;

  if (const auto *FD = dyn_cast_or_null<FunctionDecl>(Call->getDecl()))
    return !FD->hasBody();

  // A C/C++ indirect call with an unresolved target has no declaration.
  return !Call->getDecl() && isa<CallExpr>(Call->getOriginExpr());
}
...
ProgramStateRef MallocChecker::checkPointerEscapeAux(
  ...
  const bool ShouldRecordUnmodeledEscape =
      ChecksEnabled[CK_UnmodeledAllocationEscapeChecker] &&
      (Kind == PSK_DirectEscapeOnCall ||
       Kind == PSK_IndirectEscapeOnCall) &&
      callHasNoAvailableCOrCXXDefinition(Call) && !isMemCall(*Call) &&
      !isStandardNewDelete(*Call) && !getOriginCXXDeleteExpr(Call);
  ...
  for (SymbolRef Sym : Escaped) {
    ...
    if (ShouldRecordUnmodeledEscape &&
        isUnmodeledEscapeAllocationFamily(RS))
      State = State->add<PendingUnmodeledAllocationEscapes>(Sym);
...
static AllocationFamily toInternalFamily(AllocationFamilyKind Family) {
  switch (Family) {
  case AllocationFamilyKind::Malloc:
    return AF_Malloc;
  case AllocationFamilyKind::CXXNew:
    return AF_CXXNew;
  case AllocationFamilyKind::CXXNewArray:
    return AF_CXXNewArray;
  case AllocationFamilyKind::Alloca:
    return AF_Alloca;
  }
  llvm_unreachable("unknown public allocation family");
}
...
ProgramStateRef markAllocated(ProgramStateRef State, SymbolRef Sym,
  ...
  return State->set<RegionState>(
      Sym, RefState::getAllocated(toInternalFamily(Family), Origin));
}

bool isTracked(ProgramStateRef State, SymbolRef Sym) {
  return Sym && State->get<RegionState>(Sym);
}
...
bool wasReleasedBy(ProgramStateRef State, SymbolRef Sym,
  ...
  if (!Sym || !Deallocator || !isReleased(State, Sym))
    return false;
  const FunctionDecl *const *Recorded = State->get<ConfiguredDeallocator>(Sym);
  return Recorded &&
         (*Recorded)->getCanonicalDecl() == Deallocator->getCanonicalDecl();
}

ProgramStateRef markReleased(ProgramStateRef State, SymbolRef Sym,
                             const Expr *Origin) {
  return markReleased(State, Sym, Origin, nullptr);
}
...
ProgramStateRef markReleased(ProgramStateRef State, SymbolRef Sym,
  ...
  if (const RefState *RS = State->get<RegionState>(Sym))
    Family = RS->getAllocationFamily();
  State = State->set<RegionState>(Sym, RefState::getReleased(Family, Origin));
  if (Deallocator)
    return State->set<ConfiguredDeallocator>(Sym,
                                             Deallocator->getCanonicalDecl());
  return State->remove<ConfiguredDeallocator>(Sym);
}
...
ProgramStateRef markRelinquished(ProgramStateRef State, SymbolRef Sym,
  ...
  State = State->remove<ConfiguredDeallocator>(Sym);
  return State->set<RegionState>(
      Sym, RefState::getRelinquished(RS->getAllocationFamily(), Origin));
}
...
ProgramStateRef markEscaped(ProgramStateRef State, SymbolRef Sym) {
  ...
  if (!RS)
    return State;
  State = State->remove<ConfiguredDeallocator>(Sym);
  return State->set<RegionState>(Sym, RefState::getEscaped(RS));
}
...
REGISTER_CHECKER(UnmodeledAllocationEscapeChecker)
