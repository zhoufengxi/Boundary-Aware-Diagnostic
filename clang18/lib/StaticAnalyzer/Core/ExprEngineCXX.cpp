...
// Peel away the wrapper expressions that surround a lambda initializer so the
// result can be compared against the LambdaExpr itself. This mirrors the
// unwrap logic used elsewhere (IgnoreParenImpCasts plus the
// ExprWithCleanups / MaterializeTemporaryExpr / CXXBindTemporaryExpr
// getSubExpr chain). Used by VisitLambdaExpr to locate the enclosing
// DeclStmt that initializes a variable with the lambda.
...
void ExprEngine::VisitLambdaExpr(const LambdaExpr *LE, ExplodedNode *Pred,
  ...
  if (!isLambdaAssignmentFixEnabledForCurrentStandard(LangOpts)) {
    ...
    for (auto const [Idx, FieldForCapture, InitExpr] :
      ...
      if (!FieldForCapture->hasCapturedVLAType()) {
        ...
        // Capturing a 0 length array is a no-op, so we ignore it to get a
        // more accurate analysis. If it's not ignored, it would set the
        // default binding of the lambda to 'Unknown', which can lead to
        // falsely detecting 'Uninitialized' values as 'Unknown' and not
        // reporting a warning.
  ...
  // When the lambda is initialized in a single-decl DeclStmt (e.g.
  // `auto f = [...]{};`), find the enclosing DeclStmt via the AST parent
  // chain so that captures can be bound to the variable's stack region
  // instead of a temporary object region.
      ...
      if (const auto *DS = dyn_cast<DeclStmt>(P)) {
        if (DS->isSingleDecl()) {
          if (const auto *VD = dyn_cast<VarDecl>(*DS->decl_begin())) {
            // Confirm the variable's initializer (once peeled) is this LE.
  ...
  // If we found an enclosing DeclStmt and the object is not yet registered,
  // register the variable's stack lvalue as the object under construction.
  std::optional<SVal> TargetOU;
  ...
  if (TargetOU) {
    // Bind captures to the variable's stack region.
    V = *TargetOU;
  } else {
    // Fall back to a temporary object region when there is no enclosing
    // DeclStmt.
  ...
  if (auto L = V.getAs<Loc>()) {
    // Convert the variable's address loc into an rvalue of the lambda type,
    // consistent with handleConstructor's handling.
