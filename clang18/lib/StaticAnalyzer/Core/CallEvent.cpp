...
static bool CSAIsStdOrSystemLikeFunctionForCTUPrune(const FunctionDecl *FD) {
  ...
  if (Name.starts_with("__builtin_") || Name == "abort" ||
...
ProgramStateRef CallEvent::invalidateRegions(unsigned BlockCount,
  ...
  return invalidateRegions(BlockCount, Orig, nullptr);
}

ProgramStateRef CallEvent::invalidateRegions(
    unsigned BlockCount, ProgramStateRef Orig,
    RegionAndSymbolInvalidationTraits *CustomTraits) const {
  ...
  RegionAndSymbolInvalidationTraits LocalTraits;
  RegionAndSymbolInvalidationTraits &ETraits =
      CustomTraits ? *CustomTraits : LocalTraits;
  ...
  for (; I != E && Idx < NumArgs; ++I, ++Idx) {
    ...
    if (custom_analyzer_fixes::isForEachLambdaArgumentFixEnabled() &&
      ...
      if (const auto *LE = dyn_cast_or_null<LambdaExpr>(Peeled)) {
        ...
        const MemRegion *TmpR = MRMgr.getCXXTempObjectRegion(LE, CallerLC);
...
RuntimeDefinition AnyFunctionCall::getRuntimeDefinition() const {
  ...
  // Layer 2: NewDelete relevance-map prune.
