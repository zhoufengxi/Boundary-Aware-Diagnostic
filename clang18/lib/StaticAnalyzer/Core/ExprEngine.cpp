...
static bool CSA_HOLDER18_BIND_DBG() {
  return std::getenv("CSA_HOLDER18_BIND_DBG") != nullptr;
}
...
ProgramStateRef ExprEngine::processPointerEscapedOnBind(
  ...
  for (const std::pair<SVal, SVal> &LocAndVal : LocAndVals) {
    ...
    if (!MR->hasStackStorage()) {
      // With the struct-subobject-invalidation fix disabled, keep the
      // upstream behavior: binding to a non-stack region is an immediate
      // escape (case (2)).
