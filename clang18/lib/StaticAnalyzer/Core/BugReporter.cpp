...
static const ExplodedNode *
getErrorNodeForPathSelection(const BugReport *R) {
  const auto *PSR = dyn_cast_or_null<PathSensitiveBugReport>(R);
  if (!PSR)
    return nullptr;
...
static BugReport *
preferCallbackAwareLeakReport(BugReport *Selected,
...
BugPathGetter::BugPathGetter(const ExplodedGraph *OriginalGraph,
  ...
  //
  // Prefer reports that have a preferred path node. This is used for leak
  // reports whose error node has multiple predecessor paths, one of which
  // goes through an inlined callback/lambda path that is more useful
  // diagnostically.
...
BugPathInfo *BugPathGetter::getNextBugPath() {
  ...
  while (true) {
    ...
    // with the lowest BFS number.  If a preferred path node is set, prefer a
    // predecessor that can still reach it before falling back to the lowest
    // BFS number.
    ...
    if (Preferred && OrigN != Preferred) {
      for (auto PI = OrigN->pred_begin(), PE = OrigN->pred_end(); PI != PE;
           ++PI) {
...
std::optional<PathDiagnosticBuilder> PathDiagnosticBuilder::findValidReport(
  ...
  while (BugPathInfo *BugPath = BugGraph.getNextBugPath()) {
    ...
    //
    // Preferred callback leak reports may have their error node inside the
    // STL algorithm implementation, while their diagnostically relevant
    // path goes through the user callback/lambda. The generic
    // likely-false-positive suppression can incorrectly reject these
    // reports before the preferred callback path is emitted.
...
void BugReporter::FlushReport(BugReportEquivClass& EQ) {
  ...
  std::optional<PathDiagnosticLocation> SharedAllocLoc;
  if (custom_analyzer_fixes::isReportPathFixEnabled())
