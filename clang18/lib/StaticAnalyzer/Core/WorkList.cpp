...
static bool isConfiguredCallbackContinuation(const WorkListUnit &U) {
  const ExplodedNode *N = U.getNode();
  return N && N->getState()->get<ConfiguredCallbackContinuation>();
}
...
public:
  ...
  void enqueue(const WorkListUnit &U) override {
    ...
    if (!BE) {
      ...
      const bool WasUnexplored = Reachable.insert(LocId).second;
      const bool Prioritize = isConfiguredCallbackContinuation(U);

      if (WasUnexplored || Prioritize) {
        if (Prioritize)
        StackUnexplored.push_back(U);
      } else {
...
public:
  ...
  void enqueue(const WorkListUnit &U) override {
    ...
    if (auto BE = N->getLocation().getAs<BlockEntrance>()) {
      ...
      if (isConfiguredCallbackContinuation(U)) {
        NumVisited = 0;
      }
    }
