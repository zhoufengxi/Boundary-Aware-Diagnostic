...
private:
  ...
  // Syntax-only information is shared by every path that reaches a call.
  // Cache it on the engine instead of rescanning arguments at every node.
  llvm::DenseMap<const CallExpr *,
                 llvm::SmallVector<const LambdaExpr *, 2>>
      LambdaArgumentCache;

  // Functions that have actually been scheduled as synthetic uv work
  // callbacks. This provides a pointer-only fast path on ordinary call exits.
  llvm::SmallPtrSet<const FunctionDecl *, 4> UvWorkCallbackDecls;

  // Functions scheduled from configuration-driven callback sequences.
  // This is a pointer-only fast path on unrelated call exits.
  llvm::SmallPtrSet<const FunctionDecl *, 8> ConfiguredCallbackDecls;
...
public:
  ...
  // MY: simplified public entry to mark an object under construction, used by
  // lambda/UV-callback modeling (ported from LLVM15).
  ProgramStateRef startObjectConstruction(ProgramStateRef State,
...
private:
  ...
  // MY: UV async-callback modeling (ported from LLVM15). Definitions live in
  // ExprEngineCallAndReturn.cpp; state traits in UvAsyncModeling.h.
  void processPendingUvCallbackLogOnly(ExplodedNodeSet &Dst,
                                       const ExplodedNodeSet &Src,
                                       const CallEvent &Call);
  void processPendingUvAfterCallbackOnWorkExit(ExplodedNodeSet &Dst,
                                               const ExplodedNodeSet &Src,
                                               const FunctionDecl *ExitedFD,
                                               const Expr *CallE);
  void processPendingUvCallback(ExplodedNodeSet &Dst,
                                const ExplodedNodeSet &Src,
                                const CallEvent &Call);
  void inlinePendingUvCallback(ExplodedNodeSet &Dst, ExplodedNode *Pred,
                               ProgramStateRef State, const Expr *CallE,
                               const FunctionDecl *CallbackFD,
                               const MemRegion *ReqR);

  void processPendingConfiguredCallbacks(ExplodedNodeSet &Dst,
                                         const ExplodedNodeSet &Src);
  void processConfiguredCallbackExit(ExplodedNodeSet &Dst,
                                     const ExplodedNodeSet &Src,
                                     const FunctionDecl *ExitedFD,
                                     const StackFrameContext *ExitedFrame);
  void scheduleNextConfiguredCallback(
