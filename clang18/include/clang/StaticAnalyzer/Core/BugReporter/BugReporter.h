...
public:
  ...
  // MY: leak-report path selection (ported from LLVM15).
  const ExplodedNode *PreferredPathNode = nullptr;
  SymbolRef LeakSymbolForPathSelection = nullptr;
...
public:
  ...
  // MY: leak-report path selection accessors (ported from LLVM15).
  void setLeakSymbolForPathSelection(SymbolRef Sym) {
    LeakSymbolForPathSelection = Sym;
  }

  SymbolRef getLeakSymbolForPathSelection() const {
    return LeakSymbolForPathSelection;
  }

  void setPreferredPathNode(const ExplodedNode *N) { PreferredPathNode = N; }

  const ExplodedNode *getPreferredPathNode() const {
    return PreferredPathNode;
  }

  bool hasPreferredPathNode() const { return PreferredPathNode != nullptr; }
