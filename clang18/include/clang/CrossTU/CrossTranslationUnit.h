...
private:
  ...
  public:
    // MY: expose cached ASTUnits for CTU template-spec fallback lookup
    // (ported from LLVM15).
    const OwningMapTy &getFileASTUnitMap() const { return FileASTUnitMap; }
  };

  ASTUnitStorage ASTStorage;

  // The template fallback is queried from the exploded-graph hot path.  Keep
  // all of its caches on the CTU context so they cannot leak across analyses.
  using FallbackFunctionIndexTy =
      llvm::StringMap<const clang::FunctionDecl *>;
  llvm::DenseMap<const clang::ASTUnit *,
                 std::unique_ptr<FallbackFunctionIndexTy>>
      FallbackFunctionIndexes;
  llvm::StringMap<const clang::FunctionDecl *> FallbackDefinitionCache;
  llvm::StringMap<unsigned> FallbackMissGeneration;
  llvm::DenseMap<const clang::FunctionDecl *, unsigned>
      FallbackDeclFailureGeneration;
  unsigned FallbackCacheGeneration = 0;
  unsigned FallbackKnownASTUnitCount = 0;
  llvm::StringSet<> ExternalASTLoadFailureCache;
};
