...
static bool CSA_LAMBDA_DBG() {
  return std::getenv("CSA_LAMBDA_DBG") != nullptr && 0;
}
...
static bool CSAHasDeclContext(const Decl *D) {
  return D && D->getDeclContext() != nullptr;
}

static bool CSADeclLocContains(const Decl *D, StringRef Needle) {
  if (!D || !CSAHasDeclContext(D))
    return false;
  ...
  return File.contains(Needle);
}

static bool CSAInterestingDecl(const Decl *D) {
  if (!D)
    return false;

  if (CSADeclLocContains(D, "ipc_remote_object_internal.cpp"))
    return true;
  if (CSADeclLocContains(D, "process_skeleton.cpp"))
    return true;
  if (CSADeclLocContains(D, "__memory/shared_ptr.h"))
    return true;
  if (CSADeclLocContains(D, "ipc_debug.h"))
    return true;

  return false;
}

static bool CSAHasLexicalDeclContext(const Decl *D) {
  return D && D->getLexicalDeclContext() != nullptr;
}
...
///////
static bool isInSystemHeader(const Decl *D) {
  if (!D)
    return false;
  const auto &SM = D->getASTContext().getSourceManager();
  SourceLocation L = D->getLocation();
  return L.isValid() && SM.isInSystemHeader(SM.getSpellingLoc(L));
}
...
static thread_local unsigned CSA_CTU_FFRT_IMPORT_DEPTH = 0;
static thread_local llvm::DenseMap<const Decl *, unsigned>
    CSA_CTU_ActiveFunctionImportCount;

static constexpr unsigned CSA_CTU_FunctionImportReentryLimit = 16;

static bool shouldCountFunctionImportReentry(const Decl *D) {
  if (!D)
    return false;
...
static bool CSANeedFTSpecBodyMerge(const FunctionDecl *FromFD,
                                   const FunctionDecl *ToFD) {
  ...
  return FromFD->getTemplatedKind() ==
             FunctionDecl::TK_FunctionTemplateSpecialization &&
         ToFD->getTemplatedKind() ==
             FunctionDecl::TK_FunctionTemplateSpecialization &&
         FromFD->doesThisDeclarationHaveABody() &&
         !ToFD->doesThisDeclarationHaveABody();
}
...
class ASTNodeImporter : public TypeVisitor<ASTNodeImporter, ExpectedType>,
  ...
  GetImportedOrCreateSpecialDecl(ToDeclT *&ToD, CreateFunT CreateFun,
    ...
    if (ToD) {
      ...
      bool NeedBodyMerge = false;
      if (const auto *FromFD = dyn_cast_or_null<FunctionDecl>(FromD)) {
        if (const auto *ToFD = dyn_cast_or_null<FunctionDecl>(ToD)) {
          NeedBodyMerge = CSANeedFTSpecBodyMerge(FromFD, ToFD);
        }
      }
...
Error ASTNodeImporter::ImportDeclContext(Decl *FromD, DeclContext *&ToDC,
  ...
  if (FromD && FromD->getDeclContext()) {
    if (auto *CtxDecl = llvm::dyn_cast<Decl>(FromD->getDeclContext())) {
    }
  }

  if (FromD && FromD->getLexicalDeclContext()) {
    if (auto *LexDecl = llvm::dyn_cast<Decl>(FromD->getLexicalDeclContext())) {
    }
  }
  auto ToDCOrErr = Importer.ImportContext(FromD->getDeclContext());
  if (!ToDCOrErr) {
...
ExpectedDecl ASTNodeImporter::VisitFunctionDecl(FunctionDecl *D) {
  ...
  auto ImportBodyIntoExistingFTSpec =
    [&](FunctionDecl *ToFD) -> ExpectedDecl {


  if (!CSANeedFTSpecBodyMerge(D, ToFD)) {

    return ToFD;
  }

  if (D->getNumParams() != ToFD->getNumParams()) {
    ASTImportError Err(ASTImportError::UnsupportedConstruct);
    return llvm::make_error<ASTImportError>(Err);
  }

  // Important:
  // When importing a body into an existing FunctionDecl, do not create and add
  // new ParmVarDecls. Instead, map From params to the existing To params so
  // DeclRefExprs inside the imported body resolve to the existing parameters.
  for (unsigned I = 0, E = D->getNumParams(); I != E; ++I) {
    ParmVarDecl *FromParam = D->getParamDecl(I);
    ParmVarDecl *ToParam = ToFD->getParamDecl(I);
    ...
    Importer.MapImported(FromParam, ToParam);
  }

  Error BodyErr = ImportFunctionDeclBody(D, ToFD);
  if (BodyErr) {
  ...
  // CSA CTU fix:
  // If ImportDeclParts finds an existing ToFD without body, and From D has
  // body, then we are not creating a new redeclaration. We are only completing
  // the existing declaration with a body. In that case we must not reconnect
  // the redecl chain with setPreviousDecl(), because the existing ToFD may
  // already participate in a lazy redecl chain.
  FunctionDecl *BodyMergeTarget = nullptr;
  bool IsBodyMergeIntoExistingDecl = false;

if (ToD) {
  if (auto *ToFD = dyn_cast<FunctionDecl>(ToD)) {
    if (CSANeedFTSpecBodyMerge(D, ToFD)) {
      return ImportBodyIntoExistingFTSpec(ToFD);
    }
  }
  ...
  FromFT = D->getDescribedFunctionTemplate();
...
if (FunctionDecl *FoundFunction = *FoundFunctionOrErr) {
  if (Decl *Def = FindAndMapDefinition(D, FoundFunction)) {
    if (auto *DefFD = dyn_cast<FunctionDecl>(Def)) {
      if (CSANeedFTSpecBodyMerge(D, DefFD)) {
        return ImportBodyIntoExistingFTSpec(DefFD);
      }
    }
  ...
  if (auto *FromConstructor = dyn_cast<CXXConstructorDecl>(D)) {
    ...
    bool EarlyReturn = GetImportedOrCreateDecl(
        ToFunction, D, Importer.getToContext(), DC, ToInnerLocStart, NameInfo,
        T, TInfo, D->getStorageClass(), D->UsesFPIntrin(),
  ...
  // Importer.MapImported(D, ToFunction);
  // Connect the redecl chain.
  // Connect the redecl chain.
  //
  // CSA CTU fix:
  // If we are only merging a body into an existing FunctionDecl, then
  // ToFunction is not a newly-created redeclaration. Its redecl chain may
  // already be connected, possibly through a lazy external redecl chain.
  // Reconnecting it with setPreviousDecl() can corrupt the redecl chain or
  // trigger a crash in LazyGenerationalUpdatePtr::makeValue().
    if (FoundByLookup) {
      auto *Recent =
          const_cast<FunctionDecl *>(FoundByLookup->getMostRecentDecl());

      if (ToFunction != Recent) {
...
ExpectedDecl ASTNodeImporter::VisitVarDecl(VarDecl *D) {
  // llvm::errs() << "\n[VVD] VisitVarDecl begin\n";
  // llvm::errs() << "[VVD] step 0 enter\n";
  // llvm::errs() << "[VVD] D ptr=" << (const void *)D << "\n";
  // llvm::errs() << "[VVD] step 1\n";
  // llvm::errs() << "[VVD] D->getDeclContext ptr="
  //              << (const void *)D->getDeclContext() << "\n";
  // llvm::errs() << "[VVD] step 2\n";
  // llvm::errs() << "[VVD] D->getLexicalDeclContext ptr="
  //              << (const void *)D->getLexicalDeclContext() << "\n";
  // llvm::errs() << "[VVD] step 3 before redecl chain\n";

  SmallVector<Decl *, 2> Redecls = getCanonicalForwardRedeclChain(D);
  // llvm::errs() << "[VVD] step 4 redecl chain built size=" << Redecls.size()
  //              << "\n";

  auto RedeclIt = Redecls.begin();
  // llvm::errs() << "[VVD] step 5 before importing previous redecls\n";
  // Import the first part of the decl chain. I.e. import all previous
  // declarations starting from the canonical decl.
  for (; RedeclIt != Redecls.end() && *RedeclIt != D; ++RedeclIt) {
    // llvm::errs() << "[VVD] step 5.1 import previous redecl ptr="
    //  << (const void *)*RedeclIt << "\n";
    ExpectedDecl RedeclOrErr = import(*RedeclIt);
    if (!RedeclOrErr) {
      // llvm::errs() << "[VVD] step 5.2 previous redecl import failed\n";
      return RedeclOrErr.takeError();
    }
    // llvm::errs() << "[VVD] step 5.3 previous redecl import ok\n";
  }
  // llvm::errs() << "[VVD] step 6 after previous redecls\n";

  assert(*RedeclIt == D);
  // llvm::errs() << "[VVD] step 7 reached target decl in redecl chain\n";
  ...
  // llvm::errs() << "[VVD] step 10 before ImportDeclParts\n";
  if (Error Err = ImportDeclParts(D, DC, LexicalDC, Name, ToD, Loc)) {
    // llvm::errs() << "[VVD] step 10.1 ImportDeclParts failed\n";
    return std::move(Err);
  }
  // llvm::errs() << "[VVD] step 11 after ImportDeclParts\n";
  // llvm::errs() << "[VVD] DC ptr=" << (const void *)DC << "\n";
  // llvm::errs() << "[VVD] LexicalDC ptr=" << (const void *)LexicalDC << "\n";
  // llvm::errs() << "[VVD] ToD ptr=" << (const void *)ToD << "\n";

  if (ToD) {
    // llvm::errs() << "[VVD] step 12 ToD already exists, return early\n";
    return ToD;
  }
  ...
  // llvm::errs() << "[VVD] step 20 before lookup path, isFileVarDecl="
  //              << D->isFileVarDecl() << "\n";
  if (D->isFileVarDecl()) {
    SmallVector<NamedDecl *, 4> ConflictingDecls;
    unsigned IDNS = Decl::IDNS_Ordinary;
    // llvm::errs() << "[VVD] step 21 before findDeclsInToCtx\n";
    auto FoundDecls = Importer.findDeclsInToCtx(DC, Name);
    // llvm::errs() << "[VVD] step 22 after findDeclsInToCtx\n";
    for (auto *FoundDecl : FoundDecls) {
      // llvm::errs() << "[VVD] step 23 found decl ptr=" << (const void
      // *)FoundDecl
      //              << "\n";
      if (!FoundDecl->isInIdentifierNamespace(IDNS))
        continue;
      ...
      if (auto *FoundVar = dyn_cast<VarDecl>(FoundDecl)) {
        // llvm::errs() << "[VVD] step 24 found var ptr=" << (const void
        // *)FoundVar
        //              << "\n";
        if (!hasSameVisibilityContextAndLinkage(FoundVar, D))
          continue;
        // llvm::errs() << "[VVD] step 25 passed visibility/linkage\n";

        if (Importer.IsStructurallyEquivalent(D->getType(),
                                              FoundVar->getType())) {
          // llvm::errs() << "[VVD] step 26 structurally equivalent types\n";

          // The VarDecl in the "From" context has a definition, but in the
          // "To" context we already have a definition.
          VarDecl *FoundDef = FoundVar->getDefinition();
          // llvm::errs() << "[VVD] step 27 FoundDef ptr="
          //              << (const void *)FoundDef
          //              << " D is def=" << D->isThisDeclarationADefinition()
          //              << "\n";
          if (D->isThisDeclarationADefinition() && FoundDef) {
            // llvm::errs() << "[VVD] step 28 return mapped FoundDef\n";
            return Importer.MapImported(D, FoundDef);
          }
          ...
          // llvm::errs() << "[VVD] step 29 before getAnyInitializer\n";
          if (D->getInit() && FoundVar->getAnyInitializer(FoundDInit)) {
            // llvm::errs() << "[VVD] step 30 return mapped FoundDInit ptr="
            //              << (const void *)FoundDInit << "\n";
            return Importer.MapImported(D, const_cast<VarDecl *>(FoundDInit));
          }

          FoundByLookup = FoundVar;
          // llvm::errs() << "[VVD] step 31 set FoundByLookup ptr="
          //  << (const void *)FoundByLookup << "\n";
          break;
        }
        ...
        // llvm::errs() << "[VVD] step 32 after array-type check prep\n";
        if (FoundArray && TArray) {
          if (isa<IncompleteArrayType>(FoundArray) &&
              isa<ConstantArrayType>(TArray)) {
            // llvm::errs()
            //     << "[VVD] step 33 patch incomplete->constant array type\n";
            // Import the type.
            if (auto TyOrErr = import(D->getType()))
              FoundVar->setType(*TyOrErr);
            else {
              // llvm::errs() << "[VVD] step 33.1 import array type failed\n";
              return TyOrErr.takeError();
            }

            FoundByLookup = FoundVar;
            // llvm::errs() << "[VVD] step 33.2 set FoundByLookup ptr="
            //  << (const void *)FoundByLookup << "\n";
            break;
          } else if (isa<IncompleteArrayType>(TArray) &&
                     isa<ConstantArrayType>(FoundArray)) {
            FoundByLookup = FoundVar;
            // llvm::errs() << "[VVD] step 34 set FoundByLookup ptr="
            //              << (const void *)FoundByLookup << "\n";
            break;
          }
        }
        ...
        // llvm::errs()
        //     << "[VVD] step 35 emit ODR variable type inconsistent diag\n";
    ...
    // llvm::errs() << "[VVD] step 36 after lookup loop, conflicts="
    //              << ConflictingDecls.size() << "\n";
    if (!ConflictingDecls.empty()) {
      // llvm::errs() << "[VVD] step 37 before HandleNameConflict\n";
      ExpectedName NameOrErr = Importer.HandleNameConflict(
          Name, DC, IDNS, ConflictingDecls.data(), ConflictingDecls.size());
      if (NameOrErr)
        Name = NameOrErr.get();
      else {
        // llvm::errs() << "[VVD] step 37.1 HandleNameConflict failed\n";
        return NameOrErr.takeError();
      }
      // llvm::errs() << "[VVD] step 37.2 after HandleNameConflict\n";
    }
  }

  // llvm::errs() << "[VVD] step 40 before importChecked(type etc.)\n";
  Error Err = Error::success();
  auto ToType = importChecked(Err, D->getType());
  // llvm::errs() << "[VVD] step 41 after importChecked(type)\n";
  auto ToTypeSourceInfo = importChecked(Err, D->getTypeSourceInfo());
  // llvm::errs() << "[VVD] step 42 after importChecked(type source info)\n";
  auto ToInnerLocStart = importChecked(Err, D->getInnerLocStart());
  // llvm::errs() << "[VVD] step 43 after importChecked(inner loc)\n";
  auto ToQualifierLoc = importChecked(Err, D->getQualifierLoc());
  // llvm::errs() << "[VVD] step 44 after importChecked(qualifier loc)\n";
  if (Err) {
    // llvm::errs() << "[VVD] step 45 importChecked block failed\n";
    return std::move(Err);
  }

  VarDecl *ToVar;
  // llvm::errs() << "[VVD] step 50 before create decl, isDecompositionDecl="
  //              << (bool)dyn_cast<DecompositionDecl>(D) << "\n";
  if (auto *FromDecomp = dyn_cast<DecompositionDecl>(D)) {
    SmallVector<BindingDecl *> Bindings(FromDecomp->bindings().size());
    // llvm::errs() << "[VVD] step 51 before ImportArrayChecked bindings\n";
    if (Error Err =
            ImportArrayChecked(FromDecomp->bindings(), Bindings.begin())) {
      // llvm::errs() << "[VVD] step 51.1 ImportArrayChecked failed\n";
      return std::move(Err);
    }
    // llvm::errs() << "[VVD] step 52 after ImportArrayChecked bindings\n";

    DecompositionDecl *ToDecomp;
    // llvm::errs()
    //     << "[VVD] step 53 before
    //     GetImportedOrCreateDecl(DecompositionDecl)\n";
    if (GetImportedOrCreateDecl(
            ToDecomp, FromDecomp, Importer.getToContext(), DC, ToInnerLocStart,
            Loc, ToType, ToTypeSourceInfo, D->getStorageClass(), Bindings)) {
      // llvm::errs() << "[VVD] step 53.1 early return ToDecomp ptr="
      //              << (const void *)ToDecomp << "\n";
      return ToDecomp;
    }
    // llvm::errs()
    ...
    // llvm::errs() << "[VVD] step 55 before
    // GetImportedOrCreateDecl(VarDecl)\n";
    if (GetImportedOrCreateDecl(ToVar, D, Importer.getToContext(), DC,
                                ToInnerLocStart, Loc,
                                Name.getAsIdentifierInfo(), ToType,
                                ToTypeSourceInfo, D->getStorageClass())) {
      // llvm::errs() << "[VVD] step 55.1 early return ToVar ptr="
      //              << (const void *)ToVar << "\n";
      return ToVar;
    }
    // llvm::errs() << "[VVD] step 56 after GetImportedOrCreateDecl(VarDecl)\n";
  }

  // llvm::errs() << "[VVD] step 57 ToVar ptr=" << (const void *)ToVar << "\n";
  ToVar->setTSCSpec(D->getTSCSpec());
  // llvm::errs() << "[VVD] step 58 after setTSCSpec\n";
  ToVar->setQualifierInfo(ToQualifierLoc);
  // llvm::errs() << "[VVD] step 59 after setQualifierInfo\n";
  ToVar->setAccess(D->getAccess());
  // llvm::errs() << "[VVD] step 60 after setAccess\n";
  ToVar->setLexicalDeclContext(LexicalDC);
  // llvm::errs() << "[VVD] step 61 after setLexicalDeclContext\n";

  if (FoundByLookup) {
    // llvm::errs() << "[VVD] step 62 FoundByLookup ptr="
    //              << (const void *)FoundByLookup << "\n";
    auto *Recent = const_cast<VarDecl *>(FoundByLookup->getMostRecentDecl());
    // llvm::errs() << "[VVD] step 63 MostRecentDecl ptr=" << (const void
    // *)Recent
    //              << "\n";
    ToVar->setPreviousDecl(Recent);
    // llvm::errs() << "[VVD] step 64 after setPreviousDecl\n";
  }

  // Import the described template, if any.
  // llvm::errs() << "[VVD] step 70 has described var template="
  //              << (D->getDescribedVarTemplate() != nullptr) << "\n";
  if (D->getDescribedVarTemplate()) {
    // llvm::errs() << "[VVD] step 71 before import described var template\n";
    auto ToVTOrErr = import(D->getDescribedVarTemplate());
    if (!ToVTOrErr) {
      // llvm::errs() << "[VVD] step 71.1 import described var template
      // failed\n";
      return ToVTOrErr.takeError();
    }
    // llvm::errs() << "[VVD] step 72 after import described var template\n";
  }

  // llvm::errs() << "[VVD] step 80 before ImportInitializer\n";
  if (Error Err = ImportInitializer(D, ToVar)) {
    // llvm::errs() << "[VVD] step 80.1 ImportInitializer failed\n";
    return std::move(Err);
  }
  // llvm::errs() << "[VVD] step 81 after ImportInitializer\n";

  // llvm::errs() << "[VVD] step 82 isConstexpr=" << D->isConstexpr() << "\n";
  if (D->isConstexpr()) {
    ToVar->setConstexpr(true);
    // llvm::errs() << "[VVD] step 83 after setConstexpr\n";
  }

  // llvm::errs() << "[VVD] step 90 before addDeclToContexts\n";
  addDeclToContexts(D, ToVar);
  // llvm::errs() << "[VVD] step 91 after addDeclToContexts\n";

  // Import the rest of the chain. I.e. import all subsequent declarations.
  // llvm::errs() << "[VVD] step 100 before importing subsequent redecls\n";
  for (++RedeclIt; RedeclIt != Redecls.end(); ++RedeclIt) {
    // llvm::errs() << "[VVD] step 100.1 import subsequent redecl ptr="
    //  << (const void *)*RedeclIt << "\n";
    ExpectedDecl RedeclOrErr = import(*RedeclIt);
    if (!RedeclOrErr) {
      // llvm::errs() << "[VVD] step 100.2 subsequent redecl import failed\n";
      return RedeclOrErr.takeError();
    }
    // llvm::errs() << "[VVD] step 100.3 subsequent redecl import ok\n";
  }

  // llvm::errs() << "[VVD] VisitVarDecl end\n";
  return ToVar;
}
...
Expected<Decl *> ASTImporter::Import(Decl *FromD) {
  ...
  if (ToD) {
    ...
    bool NeedDeepReimport = false;

    if (const auto *FromFD = dyn_cast<FunctionDecl>(FromD)) {
      if (const auto *ToFD = dyn_cast<FunctionDecl>(ToD)) {
        NeedDeepReimport = CSANeedFTSpecBodyMerge(FromFD, ToFD);
      }
    }
        // If FromD has some updated flags after last import, apply it.
    updateFlags(FromD, ToD);

    bool HasCycle = ImportPath.hasCycleAtBack();

    if (HasCycle) {
      SavedImportPaths[FromD].push_back(ImportPath.copyCycleAtBack());
    }

    if (HasCycle && NeedDeepReimport) {
  ...
  //
  // CSA/CTU defensive guard:
  // In large C++17 projects, importing a CTU definition may recursively import
  // the same function declaration through template return types, auto types,
  // constexpr expressions, or non-type template arguments. Some recursion is
  // normal for C++ templates, so do not fail on the first reentry. Only stop
  // when the same function with a body is imported too many times in the
  // current importer call stack.
  const bool CountFunctionReentry =
      custom_analyzer_fixes::isCTUTemplateInstantiationFixEnabled() &&
      shouldCountFunctionImportReentry(FromD);

  if (CountFunctionReentry) {
    unsigned &Count = CSA_CTU_ActiveFunctionImportCount[FromD];
    ++Count;

    if (Count > CSA_CTU_FunctionImportReentryLimit) {
      --Count;
      if (Count == 0)
        CSA_CTU_ActiveFunctionImportCount.erase(FromD);
  ...
  auto FunctionReentryCountGuard = llvm::make_scope_exit([&]() {
    if (!CountFunctionReentry)
      return;

    auto It = CSA_CTU_ActiveFunctionImportCount.find(FromD);
    if (It == CSA_CTU_ActiveFunctionImportCount.end())
      return;

    assert(It->second > 0 && "bad function import recursion counter");
    --It->second;

    if (It->second == 0)
      CSA_CTU_ActiveFunctionImportCount.erase(It);
  });
...
//     // If FromD has some updated flags after last import, apply it.
//     updateFlags(FromD, ToD);
//     // If we encounter a cycle during an import then we save the relevant
//     part
...
//       // ImportedDecls and ImportedFromDecls are not symmetric.  It may
//       happen
//       // (e.g. with namespaces) that several decls from the 'from' context
//       are
//       // mapped to the same decl in the 'to' context.  If we removed entries
//       // from the LookupTable here then we may end up removing them multiple
//       // times.
...
//       if (PosF != ImportedFromDecls.end()) {
//         // In the case of TypedefNameDecl we create the Decl first and only
//         // then we import and set its DeclContext. So, the DC might not be
//         set
//         // when we reach here.
//         if (ToD->getDeclContext())
//           SharedState->removeDeclFromLookup(ToD);
...
//         if (FromDi == FromD)
//           continue;
//         // We should not set import error on a node and all following nodes
//         in
//         // the path if child import errors are ignored.
//         if (ChildErrorHandlingStrategy(FromDi).ignoreChildErrorOnParent(
//                 PrevFromDi))
...
//         setImportDeclError(FromDi, ErrOut);
//         // FIXME Should we remove these Decls from ImportedDecls?
//         //  Set the error for the mapped to Decl, which is in the "to"
//         context. auto Ii = ImportedDecls.find(FromDi); if (Ii !=
//         ImportedDecls.end())
//           SharedState->setImportDeclError(Ii->second, ErrOut);
//         // FIXME Should we remove these Decls from the LookupTable,
//         // and from ImportedFromDecls?
...
// Expected<DeclarationName> ASTImporter::HandleNameConflict(DeclarationName
// Name,
//                                                           DeclContext *DC,
//                                                           unsigned IDNS,
//                                                           NamedDecl **Decls,
...
Expected<DeclarationName> ASTImporter::HandleNameConflict(DeclarationName Name,
  ...
  if (ODRHandling == ODRHandlingType::Conservative) {
    ...
    for (unsigned I = 0; I < NumDecls; ++I) {
      if (!Decls[I] || !isInSystemHeader(Decls[I])) {
        AllSystem = false;
        break;
      }
    }
