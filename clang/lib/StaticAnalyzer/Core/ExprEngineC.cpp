...
static const Expr *peelInit(const Expr *E) {
  while (E) {
...
void ExprEngine::VisitDeclStmt(const DeclStmt *DS, ExplodedNode *Pred,
  ...
  if (CSA_LAMBDA_DBG()) {
    if (VD->getName() == "f") {
      llvm::errs() << "[CSA-LDBG] VisitDeclStmt: VarDecl f, DS="
                   << (const void *)DS << " VD=" << (const void *)VD << "\n";
      llvm::errs() << "[CSA-LDBG]   f type=" << VD->getType().getAsString()
                   << "\n";
    }
  }
  ...
  for (ExplodedNodeSet::iterator I = dstPreVisit.begin(), E = dstPreVisit.end();
    ...
    if (CSA_LAMBDA_DBG() && VD->getName() == "f") {
      SVal FLV = state->getLValue(VD, LC);
      dumpSVal("[VARINIT] f lvalue", FLV);
      if (auto L = FLV.getAs<Loc>()) {
        const MemRegion *FR = L->getAsRegion();
        llvm::errs() << "[CSA-LDBG] [VARINIT] f region=" << (const void *)FR
                     << " (" << FR << ")\n";
      }
    }
    ...
    if (const Expr *InitEx = VD->getInit()) {
      ...
      ///////////////////////////////////////////////////////
      // const Expr *Raw = InitEx;

      // if (const auto *LE = dyn_cast_or_null<LambdaExpr>(Peeled)) {
      //   llvm::errs() << "[CSA-LDBG] [VARINIT] f InitEx(raw)="
      //                << (const void *)Raw
      //                << " class=" << Raw->getStmtClassName() << "\n";
      //   llvm::errs() << "[CSA-LDBG] [VARINIT] f InitEx(peeled)="
      //                << (const void *)Peeled << " class="
      //                << (Peeled ? Peeled->getStmtClassName() : "<null>")
      //                << "\n";
      //   llvm::errs() << "[CSA-LDBG] [VARINIT] peeled is LambdaExpr LE="
      //                << (const void *)LE << "\n";
      //   if (isa_and_nonnull<LambdaExpr>(Peeled)) {
      //     llvm::errs()
      //         << "[CSA-LDBG] [VARINIT] f init is LambdaExpr (peeled)\n";
      //   }

      //   SVal FLV = state->getLValue(VD, LC);


      //   state = startObjectConstruction(state, ConstructionContextItem(DS),
      //   LC,
      //                                   FLV);

      //   if (CSA_LAMBDA_DBG() && VD->getName() == "f") {
      //     llvm::errs() << "[CSA-LDBG] [VARINIT] Lambda init: "
      //                     "startObjectConstruction(DS)->&f\n";
      //     dumpSVal("[VARINIT]  f lvalue", FLV);
      //   }
      ...
      if (CSA_LAMBDA_DBG() && VD->getName() == "f") {
        const Expr *Raw = InitEx;
        const Expr *Peeled = peelInitExpr(InitEx);
        ...
        if (const auto *LE = dyn_cast_or_null<LambdaExpr>(Peeled)) {
          llvm::errs() << "[CSA-LDBG] [VARINIT] peeled is LambdaExpr LE="
                       << (const void *)LE << "\n";
          if (auto *LCD = LE->getLambdaClass())
            llvm::errs() << "[CSA-LDBG] [VARINIT] lambda class="
                         << LCD->getQualifiedNameAsString() << "\n";
        } else {
          llvm::errs() << "[CSA-LDBG] [VARINIT] peeled is NOT LambdaExpr\n";
        }
        if (isa_and_nonnull<LambdaExpr>(Peeled)) {
          llvm::errs()
              << "[CSA-LDBG] [VARINIT] f init is LambdaExpr (peeled)\n";
        ...
        dumpSVal("[VARINIT] InitVal", InitVal);
      }
    

      assert(DS->isSingleDecl());
      Optional<SVal> OU = getObjectUnderConstruction(state, DS, LC);

      if (CSA_LAMBDA_DBG() && VD->getName() == "f") {
        if (OU) {
          llvm::errs()
              << "[CSA-LDBG] [VARINIT] getObjectUnderConstruction(DS) HIT\n";
          dumpSVal("[VARINIT] OU(DS)", *OU);
          if (auto L = OU->getAs<Loc>()) {
            const MemRegion *OR = L->getAsRegion();
            llvm::errs() << "[CSA-LDBG] [VARINIT] OU region="
                         << (const void *)OR << " (" << OR << ")\n";
          }
        } else {
          llvm::errs()
              << "[CSA-LDBG] [VARINIT] getObjectUnderConstruction(DS) MISS\n";
        }
      }
      ...
      if (OU) {
        ...
        if (CSA_LAMBDA_DBG() && VD->getName() == "f") {
          llvm::errs() << "[CSA-LDBG] [VARINIT] finishObjectConstruction(DS) done; no bind\n";
        }
        ...
        if (InitVal.isUnknown()) {
          ...
          InitVal = svalBuilder.conjureSymbolVal(nullptr, InitEx, LC, Ty,
                                                 currBldrCtx->blockCount());
          if (CSA_LAMBDA_DBG() && VD->getName() == "f") {
            dumpSVal("[VARINIT] after recovery InitVal", InitVal);
            SVal DstLoc = state->getLValue(VD, LC);
            dumpSVal("[VARINIT] evalBind dstLoc", DstLoc);
          }
        }
        ...
        if (CSA_LAMBDA_DBG() && VD->getName() == "f") {
          SVal DstLoc = state->getLValue(VD, LC);
          dumpSVal("[VARINIT] evalBind dstLoc", DstLoc);
          dumpSVal("[VARINIT] evalBind val", InitVal);
        }
