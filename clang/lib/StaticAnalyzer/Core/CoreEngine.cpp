...
bool CoreEngine::ExecuteWorkList(const LocationContext *L, unsigned MaxSteps,
  ...
  auto ProcessWList = [this, UnlimitedSteps](unsigned MaxSteps, bool IsCTU) {
    ...
    while (WList->hasWork()) {
      ...
      NumSteps++;
      AnalysisProgress::recordWorkListStep(IsCTU);
  ...
  const unsigned STUSteps = ProcessWList(MaxSteps, /*IsCTU=*/false);
  ...
  if (CTUWList) {
    ...
    const unsigned CTUSteps = ProcessWList(MaxCTUSteps, /*IsCTU=*/true);
...
void CoreEngine::enqueueEndOfFunction(ExplodedNodeSet &Set, const ReturnStmt *RS) {
  for (auto I : Set) {
    ...
    if (I->getLocationContext()->getParent()) {
      ...
      G.addEndOfPath(I);
      NumPathsExplored++;
      AnalysisProgress::recordPathCompleted();
    }
  }
}
