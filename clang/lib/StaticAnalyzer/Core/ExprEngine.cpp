...
static bool CSA_LAMBDA_DBG() {
  return std::getenv("CSA_LAMBDA_DBG") != nullptr && 0; // MY_SUP
}
...
static bool CSA_HOLDER_DBG() {
  return std::getenv("CSA_HOLDER_DBG") != nullptr;
}

static bool CSA_HOLDER_IsInterestingBindTarget(const MemRegion *MR) {
  if (!CSA_HOLDER_DBG())
    return false;
  ...
  // We are interested in bindings to object fields, such as h->ptr_ = p.
  const auto *FR = MR->getAs<FieldRegion>();
  if (!FR)
    return false;
  ...
  // Only pointer fields matter for this case.
  if (!FD->getType()->isPointerType())
    return false;

  // The Holder-like case is a field of a C++ record.
...
static bool CSA_HOLDER_IsInterestingBindValue(ProgramStateRef State, SVal V) {
  if (!CSA_HOLDER_DBG())
    return false;
...
static bool CSA_HOLDER_ShouldTraceBind(ProgramStateRef State,
                                       const MemRegion *MR,
                                       SVal V) {
  return CSA_HOLDER_IsInterestingBindTarget(MR) &&
         CSA_HOLDER_IsInterestingBindValue(State, V);
}
...
STATISTIC(NumLambdaArgumentScans,
          "The # of call expressions scanned for direct lambda arguments");
STATISTIC(NumLambdaArgumentCacheHits,
          "The # of lambda argument cache hits during dead binding removal");
...
// MY_
ProgramStateRef
ExprEngine::startObjectConstruction(ProgramStateRef State,
                                    const ConstructionContextItem &Item,
                                    const LocationContext *LC, SVal Target) {
  ConstructedObjectKey Key(Item, LC->getStackFrame());

  if (CSA_LAMBDA_DBG() && 0) {
    llvm::errs() << "[CSA-LDBG] startObjectConstruction\n";
    dumpSVal("  Target", Target);
  }

  return State->set<ObjectsUnderConstruction>(Key, Target);
}
...
//////
// Optional<SVal>
// ExprEngine::getObjectUnderConstruction(ProgramStateRef State, const Stmt *S,
//                                        const LocationContext *LCtx) {
//   return getObjectUnderConstruction(State, ConstructionContextItem(S), LCtx);
// }

// ProgramStateRef
// ExprEngine::setObjectUnderConstruction(ProgramStateRef State, const Stmt *S,
//                                        const LocationContext *LCtx,
//                                        SVal Target) {
//   return setObjectUnderConstruction(State, ConstructionContextItem(S), LCtx,
//                                     Target);
// }

// ProgramStateRef
// ExprEngine::finishObjectConstruction(ProgramStateRef State, const Stmt *S,
//                                      const LocationContext *LCtx) {
//   return finishObjectConstruction(State, ConstructionContextItem(S), LCtx);
// }
/////

// ProgramStateRef
// ExprEngine::beginObjectConstruction(ProgramStateRef State,
//                                     const ConstructionContextItem &Item,
//                                     const LocationContext *LC, SVal Target) {
//   ConstructedObjectKey Key(Item, LC->getStackFrame());
//   return State->set<ObjectsUnderConstruction>(Key, Target);
...
void ExprEngine::removeDead(ExplodedNode *Pred, ExplodedNodeSet &Out,
  ...
  auto dumpLambdaTmpPtr = [&](StringRef Where, ProgramStateRef St) {
    if (!CSA_LAMBDA_DBG())
      return;

    const CallExpr *CE = dyn_cast_or_null<CallExpr>(ReferenceStmt);
    if (!CE || CE->getNumArgs() <= 2)
      return;

    const Expr *Raw = CE->getArg(2);
    const Expr *P = peelInitExpr(Raw);
    const auto *LE = dyn_cast_or_null<LambdaExpr>(P);
    if (!LE)
      return;

    MemRegionManager &MRMgr = getSValBuilder().getRegionManager();

    const LocationContext *LocCtxt = LC;
    const MemRegion *TmpR = MRMgr.getCXXTempObjectRegion(LE, LocCtxt);

    SVal Base = loc::MemRegionVal(TmpR);

    auto It = LE->getLambdaClass()->field_begin();
    if (It == LE->getLambdaClass()->field_end())
      return;
    FieldDecl *F0 = *It;
    (void)F0;
    ++It;
    ...
    SVal PtrLoc = St->getLValue(F1, Base);

    // llvm::errs() << "[CSA-LDBG] removeDead(" << Where << ")\n";
    // llvm::errs() << "[CSA-LDBG]   RefStmt=" << (const void *)ReferenceStmt
    //              << " class="
    //              << (ReferenceStmt ? ReferenceStmt->getStmtClassName()
    //                                : "<null>")
    //              << "\n";
    // llvm::errs() << "[CSA-LDBG]   TmpR=" << (const void *)TmpR << " (" <<
    // TmpR
    //              << ")\n";
    // dumpSVal("  PtrLoc", PtrLoc);
    // if (auto L = PtrLoc.getAs<Loc>())
    //   dumpSVal("  PtrVal", St->getSVal(*L));
  };
  ...
  // not a new state.
  dumpLambdaTmpPtr("BEFORE", CleanedState);

  // Keep direct lambda temporaries alive across the pre-call purge.  The set
  // of lambda arguments is purely syntactic, so compute it once per CallExpr
  // instead of walking every argument for every exploded-graph state.
  if (custom_analyzer_fixes::isForEachLambdaArgumentFixEnabled()) {
    if (const auto *CallS = dyn_cast_or_null<CallExpr>(ReferenceStmt)) {
      auto CacheIt = LambdaArgumentCache.find(CallS);
      if (CacheIt == LambdaArgumentCache.end()) {
        llvm::SmallVector<const LambdaExpr *, 2> LambdaArgs;
        for (const Expr *Raw : CallS->arguments()) {
          if (const auto *LE =
                  dyn_cast_or_null<LambdaExpr>(peelInitExpr(Raw)))
            LambdaArgs.push_back(LE);
        }
        CacheIt = LambdaArgumentCache
                      .insert(std::make_pair(CallS, std::move(LambdaArgs)))
                      .first;
        ++NumLambdaArgumentScans;
      } else {
        ++NumLambdaArgumentCacheHits;
      }

      if (!CacheIt->second.empty()) {
        MemRegionManager &MRMgr = getSValBuilder().getRegionManager();
        for (const LambdaExpr *LE : CacheIt->second) {
          const MemRegion *TmpR = MRMgr.getCXXTempObjectRegion(LE, LC);
          SymReaper.markLive(TmpR);
        }
      }
    }
  }
  ...
  dumpLambdaTmpPtr("AFTER", CleanedState);
...
void ExprEngine::VisitCXXBindTemporaryExpr(const CXXBindTemporaryExpr *BTE,
                                           ...
                                           ExplodedNodeSet &Dst) {
  if (CSA_LAMBDA_DBG()) {
    llvm::errs() << "[CSA-LDBG] =============VisitCXXBindTemporaryExpr ENTER "
                    "BTE==========\n";
  }
  // This is a fallback solution in case we didn have a construction
  // context when we were constructing the temporary. Otherwise the map
  // should
  ...
  if (CSA_LAMBDA_DBG()) {
    const Expr *Sub = BTE->getSubExpr();
    const Expr *P = peelInitExpr(BTE);
    llvm::errs() << "[CSA-LDBG] VisitCXXBindTemporaryExpr ENTER BTE="
                 << (const void *)BTE << " sub=" << (const void *)Sub
                 << " subClass=" << (Sub ? Sub->getStmtClassName() : "<null>")
                 << " peeled=" << (const void *)P
                 << " peeledClass=" << (P ? P->getStmtClassName() : "<null>")
                 << "\n";
  }
  ...
  for (ExplodedNode *Node : PreVisit) {
    ...
    if (CSA_LAMBDA_DBG()) {
      ...
      if (LE) {
        ...
        SVal PtrLoc = State->getLValue(F1, Base);
        dumpSVal("[BTE] TmpR PtrLoc", PtrLoc);
        if (auto L = PtrLoc.getAs<Loc>())
          dumpSVal("[BTE] TmpR PtrVal", State->getSVal(*L));
      }
    }
    ...
    if (!getObjectUnderConstruction(State, BTE, LC)) {
      ...
      // those, we currently skip the CXXBindTemporaryExpr but rely on
      // adding temporary destructor nodes.
      State = addObjectUnderConstruction(State, BTE, LC, UnknownVal());
    }
    if (CSA_LAMBDA_DBG()) {
      llvm::errs() << "[CSA-LDBG] VisitBTE: PreVisitNode=" << (const void *)Node
                   << " State=" << (const void *)State.get()
                   << " LCtx=" << (const void *)LC << " PP=";
      Node->getLocation().dump();
      llvm::errs() << "\n";
    }

    ExplodedNode *NewN = StmtBldr.generateNode(BTE, Node, State);

    if (CSA_LAMBDA_DBG()) {
      llvm::errs() << "[CSA-LDBG] VisitBTE: GeneratedNode="
                   << (const void *)NewN;
      if (NewN) {
        llvm::errs() << " State=" << (const void *)NewN->getState().get()
                     << " LCtx=" << (const void *)NewN->getLocationContext()
                     << " PP=";
        NewN->getLocation().dump();
      }
      llvm::errs() << "\n";

      if (NewN) {
        dumpSVal("  Generated.getSVal(BTE)",
                 NewN->getState()->getSVal(BTE, NewN->getLocationContext()));
        if (const Expr *Sub = BTE->getSubExpr()) {
          dumpSVal("  Generated.getSVal(BTE->sub)",
                   NewN->getState()->getSVal(Sub, NewN->getLocationContext()));
        }
      }
    }

    if (CSA_LAMBDA_DBG()) {
      dumpSVal("  BTE getSVal(after)",
               State->getSVal(BTE, Node->getLocationContext()));
      if (const auto *LE = dyn_cast_or_null<LambdaExpr>(peelInitExpr(BTE)))
        dumpSVal("  LE getSVal(after)",
                 State->getSVal(LE, Node->getLocationContext()));

      llvm::errs() << "[CSA-LDBG]   VisitBTE LocCtxt="
                   << (const void *)Node->getLocationContext()
                   << " StackFrame=" << (const void *)Node->getStackFrame()
                   << "\n";
    }
  }

  if (CSA_LAMBDA_DBG()) {
    llvm::errs() << "[CSA-LDBG] =============VisitCXXBindTemporaryExpr EXIT "
                    "BTE==========\n";
  }
}
// void ExprEngine::VisitCXXBindTemporaryExpr(const CXXBindTemporaryExpr *BTE,
...
//     // Sub = peelInitExpr(Sub);

//     SVal SubV = State->getSVal(Sub, LC);

//     if (CSA_LAMBDA_DBG()) {
//       llvm::errs() << "[CSA-LDBG] VisitCXXBindTemporaryExpr: BTE="
//                    << (const void *)BTE << " sub=" << (const void *)Sub
//                    << " subClass=" << (Sub ? Sub->getStmtClassName() :
//                    "<null>")
//                    << "\n";
//       dumpSVal("  SubV(State->getSVal(Sub))", SubV);
//       dumpSVal("  Before BindExpr: BTE SVal", State->getSVal(BTE, LC));
//     }


//     if (!SubV.isUnknown())
//       State = State->BindExpr(BTE, LC, SubV);

//     if (CSA_LAMBDA_DBG()) {
//       dumpSVal("  After  BindExpr: BTE SVal", State->getSVal(BTE, LC));
//     }


//     if (!getObjectUnderConstruction(State, BTE, LC)) {

//       SVal Marker = SubV.isUnknown() ? SVal(UnknownVal()) : SubV;

//       if (CSA_LAMBDA_DBG()) {
//         llvm::errs() << "[CSA-LDBG] VisitCXXBindTemporaryExpr: addOUC(BTE)\n
//         "; dumpSVal(" Marker ", Marker);
//       }

//       State = addObjectUnderConstruction(State, BTE, LC, Marker);
//       // ---- Fix: bind BTE expr value for lambda temporaries (C++17) ----
//       const Expr *Sub = BTE->getSubExpr()->IgnoreParenImpCasts();
//       if (isa<LambdaExpr>(Sub)) {
//         // Prefer the already-computed value of Sub if available.
//         SVal SubV = State->getSVal(Sub, LC);

//         // If SubV is Unknown (can happen), read from the temp region
//         directly. if (SubV.isUnknown()) {
//           const MemRegion *TmpR =
//               svalBuilder.getRegionManager().getCXXTempObjectRegion(Sub, LC);
//           SVal TmpLoc = loc::MemRegionVal(TmpR);
//           SubV = State->getSVal(TmpLoc.castAs<Loc>(), Sub->getType());
//         }
...
//           if (CSA_LAMBDA_DBG()) {
//             llvm::errs() << "[CSA-LDBG] VisitCXXBindTemporaryExpr: "
//                             "BindExpr(BTE) for lambda\n";
//             dumpSVal("  BTE bound value", SubV);
//           }
//         }
//       }
//     }
    ...
    const MemRegion *MR = LocAndVal.first.getAsRegion();
      ...
      if (!MR->hasStackStorage()) {
        ...
        if (StoredVal != LocAndVal.second) {
          ProgramStateRef BoundState =
              State->bindLoc(loc::MemRegionVal(MR), LocAndVal.second, LCtx);

          // If the store can represent this binding, the value is now tracked
          // through the destination region. In this case, binding to a non-stack
          // region is not necessarily an immediate escape.
          if (BoundState != State)
            continue;
        }
