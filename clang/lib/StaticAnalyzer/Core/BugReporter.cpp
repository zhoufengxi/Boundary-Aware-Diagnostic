...
static bool CSA_LEAK_PATH_SELECT_DBG() {
  return std::getenv("CSA_LEAK_PATH_SELECT_DBG") != nullptr && 0;
}

static bool isLambdaCallOperatorDeclForBR(const Decl *D) {
...
// static bool isStdForEachLikeDeclForBR(const Decl *D) {
...
static bool isStdCallbackAlgorithmDeclForBR(const Decl *D) {
  const auto *ND = dyn_cast_or_null<NamedDecl>(D);
  if (!ND)
    return false;
  ...
  return Q.contains("for_each") || Q.contains("transform");
}

static bool locCtxStackHasLambdaForBR(const LocationContext *LCtx) {
  for (const LocationContext *LC = LCtx; LC; LC = LC->getParent())
    if (isLambdaCallOperatorDeclForBR(LC->getDecl()))
      return true;

  return false;
}

// static bool locCtxStackHasForEachForBR(const LocationContext *LCtx) {
//   for (const LocationContext *LC = LCtx; LC; LC = LC->getParent())
//     if (isStdForEachLikeDeclForBR(LC->getDecl()))
//       return true;

//   return false;
// }

static bool locCtxStackHasStdCallbackAlgorithmForBR(
    const LocationContext *LCtx) {
  for (const LocationContext *LC = LCtx; LC; LC = LC->getParent())
    if (isStdCallbackAlgorithmDeclForBR(LC->getDecl()))
      return true;

  return false;
}

static bool isLeakReportForPathSelection(const BugReport *R) {
  if (!R)
    return false;

  StringRef Desc = R->getDescription();
  if (Desc.contains_insensitive("leak"))
    return true;

  StringRef CheckerName = R->getBugType().getCheckerName();
  return CheckerName.contains("Leak") || CheckerName.contains("Leaks");
}


static bool isPreferredCallbackLeakReportForBR(PathSensitiveBugReport *R) {
  ...
  if (!isLeakReportForPathSelection(R))
    return false;

  const ExplodedNode *Preferred = R->getPreferredPathNode();
  if (!Preferred)
    return false;

  const LocationContext *LCtx = Preferred->getLocationContext();
  if (!LCtx)
    return false;

  if (!locCtxStackHasLambdaForBR(LCtx))
    return false;

  if (!locCtxStackHasStdCallbackAlgorithmForBR(LCtx))
    return false;
...
static bool explodedPathHasLambdaForBR(const ExplodedNode *N) {
  if (!N)
    return false;
  ...
  while (!Worklist.empty() && Budget--) {
    ...
    if (locCtxStackHasLambdaForBR(Cur->getLocationContext()))
      return true;
...
static bool explodedPathHasForEachForBR(const ExplodedNode *N) {
  if (!N)
    return false;
  ...
  while (!Worklist.empty() && Budget--) {
    ...
    if (locCtxStackHasStdCallbackAlgorithmForBR(Cur->getLocationContext()))
      return true;
...
static bool hasCallbackLeakNoteForBR(BugReport *R) {
  if (!R)
    return false;

  for (const auto &Note : R->getNotes()) {
    if (!Note)
      continue;

    if (Note->getString().contains("callback path"))
      return true;
  }
...
static bool hasPreferredPathNodeForBR(BugReport *R) {
  const auto *PSR = dyn_cast_or_null<PathSensitiveBugReport>(R);
  return PSR && PSR->getPreferredPathNode();
}



static const ExplodedNode *getErrorNodeForPathSelection(const BugReport *R) {
  const auto *PSR = dyn_cast_or_null<PathSensitiveBugReport>(R);
  if (!PSR)
    return nullptr;

  return PSR->getErrorNode();
}

static int scoreLeakReportPathForCallback(BugReport *R) {
    if (!isLeakReportForPathSelection(R))
    return 0;

  const ExplodedNode *N = getErrorNodeForPathSelection(R);
  if (!N)
    return 0;

  int Score = 0;

  if (explodedPathHasLambdaForBR(N))
    Score += 100;

  if (explodedPathHasForEachForBR(N))
    Score += 30;

  // Prefer the report for which MallocChecker has already attached
  // a callback-path note. This breaks ties among multiple reports
  // whose predecessor graphs all contain lambda/for_each frames.
if (hasPreferredPathNodeForBR(R))
  Score += 25;
...
static bool canReachNodeThroughPreds(const ExplodedNode *From,
                                     const ExplodedNode *Target) {
  if (!From || !Target)
    return false;
...
static BugReport *preferCallbackAwareLeakReport(BugReport *Selected,
                                                ArrayRef<BugReport *> Reports) {
  if (!custom_analyzer_fixes::isReportPathFixEnabled())
    return Selected;
  ...
  if (!isLeakReportForPathSelection(Selected))
    return Selected;

  BugReport *Best = Selected;
  int BestScore = scoreLeakReportPathForCallback(Selected);

  for (BugReport *R : Reports) {
    if (!R)
      continue;

    int Score = scoreLeakReportPathForCallback(R);

    if (CSA_LEAK_PATH_SELECT_DBG()) {
      llvm::errs() << "[LEAK-SELECT] candidate report=" << R
                   << " score=" << Score;

      if (const ExplodedNode *N = getErrorNodeForPathSelection(R)) {
        llvm::errs() << " node=" << N;

        if (const Stmt *S = N->getStmtForDiagnostics())
          llvm::errs() << " stmt=" << S->getStmtClassName();

        llvm::errs() << " hasLambda=" << explodedPathHasLambdaForBR(N)
                     << " hasForEach=" << explodedPathHasForEachForBR(N);
      }
    ...
    if (Score > BestScore ||
        (Score == BestScore && hasPreferredPathNodeForBR(R) &&
        !hasPreferredPathNodeForBR(Best))) {
      Best = R;
      BestScore = Score;
    }
  }

  if (CSA_LEAK_PATH_SELECT_DBG() && Best != Selected) {
    llvm::errs() << "[LEAK-SELECT] replace selected report"
                 << " old=" << Selected
                 << " new=" << Best
                 << " oldScore=" << scoreLeakReportPathForCallback(Selected)
                 << " newScore=" << BestScore
                 << "\n";
  }
...
getPreferredCallbackLeakEndPath(const PathSensitiveBugReport *R,
                                const SourceManager &SM) {
  if (!custom_analyzer_fixes::isReportPathFixEnabled())
    return nullptr;
  ...
  const Stmt *S = Preferred->getStmtForDiagnostics();
  if (!S)
    return nullptr;

  PathDiagnosticLocation Loc =
      PathDiagnosticLocation::createBegin(S, SM, LCtx);

  if (!Loc.isValid())
    return nullptr;

  auto Piece =
      std::make_shared<PathDiagnosticEventPiece>(Loc, R->getDescription());
...
PathDiagnosticBuilder::generate(const PathDiagnosticConsumer *PDC) const {
  ...
  if (PathDiagnosticPieceRef PreferredEnd =
          getPreferredCallbackLeakEndPath(getBugReport(), SM)) {
    LastPiece = PreferredEnd;
  } else if (EndNotes != VisitorsDiagnostics->end()) {
...
public:
  ...
  PathSensitiveBugReport *Report = nullptr;
  const ExplodedNode *ErrorNode = nullptr;
  const ExplodedNode *PreferredPathNode = nullptr;
};
...
class BugPathGetter {
  ...
  /// trimmed graph.
struct ReportNewNodeItem {
  PathSensitiveBugReport *Report = nullptr;
  const ExplodedNode *ErrorNode = nullptr;
  const ExplodedNode *PreferredPathNode = nullptr;
};

SmallVector<ReportNewNodeItem, 32> ReportNodes;
...
bool operator()(const ReportNewNodeItem &LHS,
                const ReportNewNodeItem &RHS) const {
  return (*this)(LHS.ErrorNode, RHS.ErrorNode);
}
  };
...
BugPathGetter::BugPathGetter(const ExplodedGraph *OriginalGraph,
  ...
  for (PathSensitiveBugReport *Report : bugReports) {
    ...
    const ExplodedNode *PreferredOldNode =
        custom_analyzer_fixes::isReportPathFixEnabled()
            ? Report->getPreferredPathNode()
            : nullptr;
    const ExplodedNode *PreferredNewNode = nullptr;

    if (PreferredOldNode)
      PreferredNewNode = ForwardMap.lookup(PreferredOldNode);

    ReportNewNodeItem Item;
    Item.Report = Report;
    Item.ErrorNode = NewNode;
    Item.PreferredPathNode = PreferredNewNode;

    ReportNodes.push_back(Item);
    RemainingNodes.insert(NewNode);

    if (CSA_LEAK_PATH_SELECT_DBG() && PreferredOldNode) {
      llvm::errs() << "[LEAK-FULLPATH] report=" << Report
                  << " errorNew=" << NewNode
                  << " preferredOld=" << PreferredOldNode
                  << " preferredNew=" << PreferredNewNode
                  << "\n";
    }
  }
...
// Sort the error paths from longest to shortest. getNextBugPath() uses
// pop_back(), so the report we want to try first must be placed at the back.
//
// Prefer reports that have a preferred path node. This is used for leak reports
// whose error node has multiple predecessor paths, one of which goes through
// an inlined callback/lambda path that is more useful diagnostically.
llvm::sort(ReportNodes, [this](const ReportNewNodeItem &L,
                               const ReportNewNodeItem &R) {
  const bool LP = L.PreferredPathNode != nullptr;
  const bool RP = R.PreferredPathNode != nullptr;

  if (LP != RP)
    return !LP && RP; // Non-preferred reports go earlier; preferred go later.

  return PriorityCompare<true>(PriorityMap)(L, R);
});
}
...
ReportNewNodeItem Item = ReportNodes.pop_back_val();

CurrentBugPath.Report = Item.Report;
CurrentBugPath.PreferredPathNode = Item.PreferredPathNode;

const ExplodedNode *OrigN = Item.ErrorNode;


if (CSA_LEAK_PATH_SELECT_DBG()) {
  llvm::errs() << "[LEAK-FULLPATH] selected bug path"
               << " report=" << CurrentBugPath.Report
               << " error=" << OrigN
               << " preferred=" << CurrentBugPath.PreferredPathNode
               << "\n";
}
...
const ExplodedNode *Preferred = CurrentBugPath.PreferredPathNode;
const ExplodedNode *NextPred = nullptr;

if (Preferred && OrigN != Preferred) {
  for (auto PI = OrigN->pred_begin(), PE = OrigN->pred_end(); PI != PE; ++PI) {
    const ExplodedNode *Pred = *PI;

    if (canReachNodeThroughPreds(Pred, Preferred)) {
      NextPred = Pred;

      if (CSA_LEAK_PATH_SELECT_DBG()) {
        llvm::errs() << "[LEAK-FULLPATH] choose preferred predecessor"
                     << " current=" << OrigN
                     << " pred=" << Pred
                     << " preferred=" << Preferred
                     << "\n";
...
if (!NextPred) {
  NextPred = *std::min_element(OrigN->pred_begin(), OrigN->pred_end(),
                               PriorityCompare<false>(PriorityMap));

  if (CSA_LEAK_PATH_SELECT_DBG() && Preferred) {
    llvm::errs() << "[LEAK-FULLPATH] choose default predecessor"
                 << " current=" << OrigN
                 << " pred=" << NextPred
                 << " preferred=" << Preferred
                 << "\n";
...
Optional<PathDiagnosticBuilder> PathDiagnosticBuilder::findValidReport(
  ...
  while (BugPathInfo *BugPath = BugGraph.getNextBugPath()) {
    ...
    if (CSA_LEAK_PATH_SELECT_DBG()) {
  llvm::errs() << "[LEAK-FINDVALID] TRY report=" << R
               << " errorNode=" << ErrorNode
               << " preferred=" << R->getPreferredPathNode()
               << " valid=" << R->isValid()
               << "\n";

  if (const Stmt *S = ErrorNode->getStmtForDiagnostics()) {
    llvm::errs() << "[LEAK-FINDVALID] error stmt="
                 << S->getStmtClassName()
                 << " loc=";
    S->getBeginLoc().print(
        llvm::errs(),
  ...
  if (const ExplodedNode *PN = R->getPreferredPathNode()) {
    llvm::errs() << "[LEAK-FINDVALID] preferred old node=" << PN;

    if (const Stmt *PS = PN->getStmtForDiagnostics()) {
      llvm::errs() << " stmt=" << PS->getStmtClassName()
                   << " loc=";
      PS->getBeginLoc().print(
          llvm::errs(),
          PN->getState()
              ->getStateManager()
              .getContext()
    ...
    //
    // Preferred callback leak reports may have their error node inside the STL
    // algorithm implementation, while their diagnostically relevant path goes
    // through the user callback/lambda. The generic likely-false-positive
    // suppression can incorrectly reject these reports before the preferred
    // callback path is emitted.
    if (!isPreferredCallbackLeakReportForBR(R)) {
      R->addVisitor<LikelyFalsePositiveSuppressionBRVisitor>();
    } else if (CSA_LEAK_PATH_SELECT_DBG()) {
      llvm::errs() << "[LEAK-FINDVALID] skip likely-fp suppression for preferred "
                      "callback leak report="
                  << R << "\n";
    }
    ...
    if (CSA_LEAK_PATH_SELECT_DBG()) {
  llvm::errs() << "[LEAK-FINDVALID] AFTER visitors"
               << " report=" << R
               << " valid=" << R->isValid()
               << " preferred=" << R->getPreferredPathNode()
               << "\n";
...
if (R->isValid()) {
  if (CSA_LEAK_PATH_SELECT_DBG()) {
    llvm::errs() << "[LEAK-FINDVALID] ACCEPT report=" << R
                 << " preferred=" << R->getPreferredPathNode()
                 << "\n";
  }
  ...
  return PathDiagnosticBuilder(
    ...
    if (CSA_LEAK_PATH_SELECT_DBG()) {
  llvm::errs() << "[LEAK-FINDVALID] REJECT report=" << R
               << " valid=" << R->isValid()
               << " preferred=" << R->getPreferredPathNode()
               << "\n";
}
...
// void BugReporter::emitReport(std::unique_ptr<BugReport> R) {
//   if (CSA_CTU_DBG() && R && shouldLogInterestingBugReport(*R) && 0) {
//     llvm::errs() << "\n[CTU-BR] ===== BugReporter::emitReport ENTER =====\n";
//     llvm::errs() << "[CTU-BR] report ptr=" << (const void *)R.get() << "\n";
//     llvm::errs() << "[CTU-BR] bug type desc="
//                  << R->getBugType().getDescription() << "\n";
//     llvm::errs() << "[CTU-BR] checker=" << R->getBugType().getCheckerName()
//                  << "\n";
//     llvm::errs() << "[CTU-BR] short desc=" << R->getShortDescription() <<
//     "\n"; llvm::errs() << "[CTU-BR] full desc=" << R->getDescription() <<
//     "\n"; dumpBugLoc(*R, "[CTU-BR] location="); dumpBugDecl(*R, "[CTU-BR]
//     issue decl=");
//   }
...
//   if (CSA_CTU_DBG() && R && shouldLogInterestingBugReport(*R) && 0) {
//     llvm::errs() << "[CTU-BR] ValidSourceLoc=" << ValidSourceLoc << "\n";
//   }

//   assert(ValidSourceLoc);
//   if (!ValidSourceLoc) {
//     if (CSA_CTU_DBG() && R && shouldLogInterestingBugReport(*R) && 0) {
//       llvm::errs() << "[CTU-BR] dropping report because location invalid\n";
//     }
//     return;
//   }
...
//   if (CSA_CTU_DBG() && shouldLogInterestingBugReport(*R) && 0) {
//     llvm::errs() << "[CTU-BR] profile computed\n";
//   }
...
//   if (CSA_CTU_DBG() && shouldLogInterestingBugReport(*R) && 0) {
//     llvm::errs() << "[CTU-BR] EQ lookup result="
//                  << (EQ ? "FOUND existing class" : "NEW class") << "\n";
//     llvm::errs() << "[CTU-BR] existing EQ ptr=" << (const void *)EQ << "\n";
//   }

//   if (!EQ) {
//     if (CSA_CTU_DBG() && R && shouldLogInterestingBugReport(*R) && 0) {
//       llvm::errs() << "[CTU-BR] creating new BugReportEquivClass\n";
//     }
...
//     if (CSA_CTU_DBG() && 0) {
//       llvm::errs() << "[CTU-BR] new EQ ptr=" << (const void *)EQ << "\n";
//       llvm::errs() << "[CTU-BR] EQClassesVector.size=" <<
//       EQClassesVector.size()
//                    << "\n";
//       llvm::errs() << "[CTU-BR] ===== BugReporter::emitReport EXIT =====\n";
//     }

//   } else {
//     if (CSA_CTU_DBG() && R && shouldLogInterestingBugReport(*R)) {
//       llvm::errs() << "[CTU-BR] merging report into existing EQ class\n";
//       llvm::errs() << "[CTU-BR] target EQ ptr=" << (const void *)EQ << "\n";
//     }

//     EQ->AddReport(std::move(R));

//     if (CSA_CTU_DBG() && 0) {
//       llvm::errs() << "[CTU-BR] merged into existing EQ class\n";
//       llvm::errs() << "[CTU-BR] ===== BugReporter::emitReport EXIT =====\n";
//     }
//   }
// }
...
void PathSensitiveBugReporter::emitReport(std::unique_ptr<BugReport> R) {
  if (auto PR = dyn_cast<PathSensitiveBugReport>(R.get()))
    if (const ExplodedNode *E = PR->getErrorNode()) {
      ...
      // locations and have to be discarded. Locations from model filesshould
...
// void PathSensitiveBugReporter::emitReport(std::unique_ptr<BugReport> R) {
//   if (CSA_CTU_DBG() && R && shouldLogInterestingBugReport(*R) && 0) {
//     llvm::errs() << "\n[CTU-BR-PS] ===== PathSensitiveBugReporter::emitReport
//     "
//                     "ENTER =====\n";
//     llvm::errs() << "[CTU-BR-PS] report ptr=" << (const void *)R.get() <<
//     "\n"; llvm::errs() << "[CTU-BR-PS] bug type desc="
//                  << R->getBugType().getDescription() << "\n";
//     llvm::errs() << "[CTU-BR-PS] checker=" <<
//     R->getBugType().getCheckerName()
//                  << "\n";
//     llvm::errs() << "[CTU-BR-PS] short desc=" << R->getShortDescription()
//                  << "\n";
//     llvm::errs() << "[CTU-BR-PS] full desc=" << R->getDescription() << "\n";
//     dumpBugLoc(*R, "[CTU-BR-PS] location=");
//     dumpBugDecl(*R, "[CTU-BR-PS] issue decl=");
//   }

//   if (auto PR = dyn_cast<PathSensitiveBugReport>(R.get())) {
//     if (CSA_CTU_DBG() && shouldLogInterestingBugReport(*R) && 0) {
//       llvm::errs() << "[CTU-BR-PS] PathSensitiveBugReport detected\n";
//       llvm::errs() << "[CTU-BR-PS] error node="
//                    << (const void *)PR->getErrorNode() << "\n";
//     }
...
//       if (CSA_CTU_DBG() && shouldLogInterestingBugReport(*R) && 0) {
//         llvm::errs() << "[CTU-BR-PS] E->isSink=" << E->isSink() << "\n";
//         llvm::errs() << "[CTU-BR-PS] E->tag="
//                      << (const void *)E->getLocation().getTag() << "\n";

//         const LocationContext *LCtx = E->getLocationContext();
//         const Decl *D = LCtx ? LCtx->getDecl() : nullptr;
//         if (const auto *ND = dyn_cast_or_null<NamedDecl>(D))
//           llvm::errs() << "[CTU-BR-PS] error node decl="
//                        << ND->getQualifiedNameAsString() << "\n";
//         else if (D)
//           llvm::errs() << "[CTU-BR-PS] error node decl kind="
//                        << D->getDeclKindName() << "\n";
//         else
//           llvm::errs() << "[CTU-BR-PS] error node decl=<null>\n";
//       }
...
//       if (CSA_CTU_DBG() && shouldLogInterestingBugReport(*R) && 0) {
//         llvm::errs() << "[CTU-BR-PS] DeclCtx autosynthesized="
//                      << DeclCtx->isBodyAutosynthesized() << " fromModelFile="
//                      << DeclCtx->isBodyAutosynthesizedFromModelFile() <<
//                      "\n";
//       }
...
//         if (CSA_CTU_DBG() && shouldLogInterestingBugReport(*R) && 0) {
//           llvm::errs()
//               << "[CTU-BR-PS] dropping report because body
//               autosynthesized\n";
//         }
//         return;
//       }
...
//   if (CSA_CTU_DBG() && R && shouldLogInterestingBugReport(*R) && 0) {
//     llvm::errs() << "[CTU-BR-PS] forwarding to BugReporter::emitReport\n";
//   }
...
void BugReporter::FlushReport(BugReportEquivClass &EQ) {
  ...
  report = preferCallbackAwareLeakReport(report, bugReports);
  if (!report)
    return;
  ...
  for (const std::string &CheckerOrPackage :
    ...
    if (report->getBugType().getCheckerName().startswith(CheckerOrPackage))
      return;
  }
  ...
  llvm::Optional<PathDiagnosticLocation> SharedAllocLoc;
  if (custom_analyzer_fixes::isReportPathFixEnabled())
    SharedAllocLoc = findSharedAllocationOriginForDoubleFreeWithLog(
        report, *Diagnostics, getSourceManager());
  for (auto &P : *Diagnostics) {
    ...
    if (custom_analyzer_fixes::isReportPathFixEnabled())
      promoteSharedAllocationOriginForDoubleFreeWithLog(
          report, *PD, SharedAllocLoc, getSourceManager());
