...
static bool CSA_MALLOC_OPT() {
  return std::getenv("CSA_MALLOC_OPT") != nullptr;
}

static bool isNewDeleteVisibleAllocationOnlyEnabled(CheckerContext &C) {
  return C.getAnalysisManager().options.NewDeleteVisibleAllocationOnly;
}

static bool isHeapSymRegionSVal(SVal V) {
  ...
  if (const MemRegion *Base = R->getBaseRegion())
    R = Base->StripCasts();

  const MemSpaceRegion *MS = R->getMemorySpace();
  return isa<HeapSpaceRegion>(MS);
}

static void dumpMallocOptSkip(StringRef Where, StringRef Reason, SVal V) {
  if (CSA_MALLOC_OPT()) {
    llvm::errs() << "[MALLOC-OPT] " << Where << " skip"
                 << " reason=" << Reason << " sval=";
...
static bool stmtTreeMentionsSymbol(const Stmt *S,
  ...
  if (const Expr *E = dyn_cast<Expr>(S)) {
    ...
    if (V.getAsSymbol() == Sym || V.getAsLocSymbol() == Sym)
      return true;
  }

  for (const Stmt *Child : S->children())
    if (stmtTreeMentionsSymbol(Child, State, LC, Sym))
      return true;

  return false;
}

static bool isLambdaRecordWithPointerAndNonTrivialField(
...
static const FieldRegion *getLambdaNonTrivialFieldDtorRegion(
  ...
  if (!isa_and_nonnull<CXXDestructorDecl>(Call->getDecl()))
    return nullptr;

  const MemRegion *ThisReg = IC->getCXXThisVal().getAsRegion();
  if (!ThisReg)
    return nullptr;
  ...
  if (!isLambdaRecordWithPointerAndNonTrivialField(ParentRD))
    return nullptr;

  // We are handling the destructor of a non-trivial lambda capture field,
  // such as a captured std::string. A sibling raw pointer capture must not be
  // considered escaped merely because this field is conservatively invalidated.
...
static const FieldRegion *getNonTrivialFieldDtorRegion(const CallEvent *Call) {
  if (!Call)
    return nullptr;
  ...
  // Only handle non-trivial field destructors, e.g. std::string tag.
  if (FD->getType()->isPointerType())
    return nullptr;
...
class FieldDtorReachableSymbolCollector final : public SymbolVisitor {
  llvm::SmallPtrSetImpl<SymbolRef> &Symbols;

public:
  explicit FieldDtorReachableSymbolCollector(
      llvm::SmallPtrSetImpl<SymbolRef> &Symbols)
      : Symbols(Symbols) {}
...
static bool shouldSkipNonFieldReachableEscapeFromFieldDtor(
    const CallEvent *Call, ProgramStateRef State, SymbolRef Sym) {
  const FieldRegion *DtorFieldRegion = getNonTrivialFieldDtorRegion(Call);
  if (!DtorFieldRegion)
    return false;

  llvm::SmallPtrSet<SymbolRef, 16> FieldReachableSymbols;
  FieldDtorReachableSymbolCollector Collector(FieldReachableSymbols);

  State->scanReachableSymbols(loc::MemRegionVal(DtorFieldRegion), Collector);

  // If this allocated symbol is not reachable from the field whose destructor
  // is currently being conservatively evaluated, it must not be marked escaped
  // because of this field destructor. This prevents a destructor of a field
  // such as BaseInfo::tag from escaping sibling fields such as Holder::ptr.
  return !FieldReachableSymbols.count(Sym);
}


class LambdaFieldReachableSymbolCollector final : public SymbolVisitor {
  llvm::SmallPtrSetImpl<SymbolRef> &Symbols;

public:
  explicit LambdaFieldReachableSymbolCollector(
      llvm::SmallPtrSetImpl<SymbolRef> &Symbols)
      : Symbols(Symbols) {}
...
static bool shouldSkipLambdaFieldNonReachableEscape(
    const CallEvent *Call, ProgramStateRef State, SymbolRef Sym) {
  const FieldRegion *DtorFieldRegion =
      getLambdaNonTrivialFieldDtorRegion(Call);
  if (!DtorFieldRegion)
    return false;

  llvm::SmallPtrSet<SymbolRef, 16> FieldReachableSymbols;
  LambdaFieldReachableSymbolCollector Collector(FieldReachableSymbols);
...
class LeakPathReachableSymbolCollector final : public SymbolVisitor {
  llvm::SmallPtrSetImpl<SymbolRef> &Symbols;

public:
  explicit LeakPathReachableSymbolCollector(
      llvm::SmallPtrSetImpl<SymbolRef> &Symbols)
      : Symbols(Symbols) {}
...
static bool sValCanReachSymbol(ProgramStateRef State, SVal V,
                               SymbolRef Target) {
  if (!Target)
    return false;

  if (V.getAsSymbol() == Target || V.getAsLocSymbol() == Target)
    return true;

  llvm::SmallPtrSet<SymbolRef, 16> Reachable;
  LeakPathReachableSymbolCollector Collector(Reachable);

  State->scanReachableSymbols(V, Collector);

  return Reachable.count(Target);
}

static bool stmtTreeCanReachSymbol(const Stmt *S,
                                   ProgramStateRef State,
  ...
  if (const Expr *E = dyn_cast<Expr>(S)) {
    ...
    if (sValCanReachSymbol(State, V, Target))
      return true;
  }

  for (const Stmt *Child : S->children()) {
    if (stmtTreeCanReachSymbol(Child, State, LCtx, Target))
      return true;
  }
...
static bool isInLambdaContextForLeakNote(const LocationContext *LCtx) {
  for (const LocationContext *LC = LCtx; LC; LC = LC->getParent()) {
    const auto *MD = dyn_cast_or_null<CXXMethodDecl>(LC->getDecl());
    if (MD && MD->getParent() && MD->getParent()->isLambda())
      return true;
  }
...
static const ExplodedNode *findLambdaUseNodeForLeak(const ExplodedNode *N,
                                                    SymbolRef Sym) {
  if (!N || !Sym)
    return nullptr;

  llvm::SmallPtrSet<const ExplodedNode *, 32> Seen;
  SmallVector<const ExplodedNode *, 64> Worklist;
  Worklist.push_back(N);

  unsigned Budget = 4096;

  const ExplodedNode *Best = nullptr;

  while (!Worklist.empty() && Budget--) {
    const ExplodedNode *Cur = Worklist.pop_back_val();
    if (!Cur || !Seen.insert(Cur).second)
      continue;

    const LocationContext *LCtx = Cur->getLocationContext();
    const Stmt *S = Cur->getStmtForDiagnostics();

    if (S && isInLambdaContextForLeakNote(LCtx) &&
        stmtTreeCanReachSymbol(S, Cur->getState(), LCtx, Sym)) {
      Best = Cur;
      break;
    }

    for (auto PI = Cur->pred_begin(), PE = Cur->pred_end(); PI != PE; ++PI)
...
static bool CSA_HOLDER_DBG() {
  return std::getenv("CSA_HOLDER_DBG") != nullptr && 0;
}

static bool CSA_HOLDER_RecordHasPointerField(const CXXRecordDecl *RD) {
  RD = RD ? RD->getDefinition() : nullptr;
  if (!RD)
    return false;
...
static const CXXRecordDecl *CSA_HOLDER_GetInterestingRecordFromCall(
    const CallEvent *Call) {
  if (!Call)
    return nullptr;
  llvm::errs() << "[CSA-HOLDER] Call=00000000000" "\n";

  if (const auto *MD = dyn_cast_or_null<CXXMethodDecl>(Call->getDecl())) {
    const CXXRecordDecl *RD = MD->getParent();
    if (CSA_HOLDER_RecordHasPointerField(RD))
      return RD->getDefinition();
  }

  // For operator delete calls, the callee is usually global/operator delete,
  // so inspect the origin CXXDeleteExpr instead.
  if (const auto *DE = dyn_cast_or_null<CXXDeleteExpr>(Call->getOriginExpr())) {
    QualType Ty = DE->getDestroyedType();
    const CXXRecordDecl *RD = Ty->getAsCXXRecordDecl();
    if (CSA_HOLDER_RecordHasPointerField(RD))
      return RD->getDefinition();
  }
  llvm::errs() << "[CSA-HOLDER] Call=11111111111" "\n";

  return nullptr;
}

static bool CSA_HOLDER_ShouldTraceEscape(const CallEvent *Call,
                                         const RefState *RS) {
  if (!CSA_HOLDER_DBG())
    return false;
  ...
  // This case is about new/new[]-tracked symbols.
  if (RS->getAllocationFamily() != AF_CXXNew &&
      RS->getAllocationFamily() != AF_CXXNewArray)
    return false;

  return CSA_HOLDER_GetInterestingRecordFromCall(Call) != nullptr;
}


static bool CSA_LEAK_PATH_DBG() {
  return std::getenv("CSA_LEAK_PATH_DBG") != nullptr;
}

static bool isLambdaCallOperatorDecl(const Decl *D) {
...
static bool isStdForEachLikeDecl(const Decl *D) {
  const auto *ND = dyn_cast_or_null<NamedDecl>(D);
  if (!ND)
    return false;

  std::string QName = ND->getQualifiedNameAsString();
  return StringRef(QName).contains("for_each");
}

static bool locCtxStackHasLambda(const LocationContext *LCtx) {
  for (const LocationContext *LC = LCtx; LC; LC = LC->getParent())
    if (isLambdaCallOperatorDecl(LC->getDecl()))
      return true;

  return false;
}

static bool locCtxStackHasForEach(const LocationContext *LCtx) {
  for (const LocationContext *LC = LCtx; LC; LC = LC->getParent())
    if (isStdForEachLikeDecl(LC->getDecl()))
      return true;

  return false;
}

static bool explodedPathHasLambdaFrame(const ExplodedNode *N) {
  if (!N)
    return false;
  ...
  while (!Worklist.empty() && Budget--) {
    ...
    if (locCtxStackHasLambda(Cur->getLocationContext()))
      return true;

    for (ExplodedNode::const_pred_iterator PI = Cur->pred_begin(),
                                           PE = Cur->pred_end();
         PI != PE; ++PI) {
      Worklist.push_back(*PI);
    }
...
static bool explodedPathHasForEachFrame(const ExplodedNode *N) {
  if (!N)
    return false;
  ...
  while (!Worklist.empty() && Budget--) {
    ...
    if (locCtxStackHasForEach(Cur->getLocationContext()))
      return true;
...
static void dumpSourceLocForLeakPath(SourceLocation Loc,
...
void MallocChecker::checkCXXNewOrCXXDelete(const CallEvent &Call,
  ...
  //     llvm::errs() << "\n[CTU-MC] checkCXXNewOrCXXDelete: function=" << Q
  //                  << "\n";
  //   }
  // }

  if (CSA_CTU_DBG() && 0) {
    const clang::Decl *CurDecl =
        C.getLocationContext() ? C.getLocationContext()->getDecl() : nullptr;
    if (shouldLogInterestingMallocDecl(CurDecl)) {
      llvm::errs() << "\n[CTU-MC] ===== checkCXXNewOrCXXDelete ENTER =====\n";
      dumpMallocCurrentFunction(C, "[CTU-MC] in function=");
      llvm::errs() << "[CTU-MC] state(before)=" << (const void *)State.get()
                   << "\n";

      if (const Decl *CalleeDecl = Call.getDecl()) {
        if (const auto *ND = llvm::dyn_cast<NamedDecl>(CalleeDecl))
          llvm::errs() << "[CTU-MC] callee=" << ND->getQualifiedNameAsString()
                       << "\n";
        else
          llvm::errs() << "[CTU-MC] callee decl kind="
                       << CalleeDecl->getDeclKindName() << "\n";
      } else {
        llvm::errs() << "[CTU-MC] callee=<null>\n";
      }

      llvm::errs() << "[CTU-MC] numArgs=" << Call.getNumArgs() << "\n";
      for (unsigned I = 0; I < Call.getNumArgs(); ++I) {
        llvm::errs() << "[CTU-MC] arg[" << I << "]\n";
        dumpCTUSVal("  value=", Call.getArgSVal(I));
      }

      if (const auto *Origin =
              dyn_cast_or_null<CallExpr>(Call.getOriginExpr())) {
        llvm::errs() << "[CTU-MC] origin expr class="
                     << Origin->getStmtClassName() << "\n";
      }
    }
  }
  ...
  if (CSA_CTU_DBG() && 0) {
    const clang::Decl *CurDecl =
        C.getLocationContext() ? C.getLocationContext()->getDecl() : nullptr;
    if (shouldLogInterestingMallocDecl(CurDecl)) {
      llvm::errs() << "[CTU-MC] overloaded operator="
                   << (FD ? (int)FD->getOverloadedOperator() : -1) << "\n";
    }
  }
  ...
  case OO_Delete:
    if (CSA_CTU_DBG() && 0) {
      const clang::Decl *CurDecl =
          C.getLocationContext() ? C.getLocationContext()->getDecl() : nullptr;
      if (shouldLogInterestingMallocDecl(CurDecl)) {
        llvm::errs() << "[CTU-MC] dispatch -> FreeMemAux (OO_Delete)\n";
      }
    }
    ...
    if (CSA_CTU_DBG() && 0) {
      const clang::Decl *CurDecl =
          C.getLocationContext() ? C.getLocationContext()->getDecl() : nullptr;
      if (shouldLogInterestingMallocDecl(CurDecl)) {
        llvm::errs() << "[CTU-MC] state(after OO_Delete)="
  ...
  case OO_Array_Delete:
    if (CSA_CTU_DBG() && 0) {
      const clang::Decl *CurDecl =
          C.getLocationContext() ? C.getLocationContext()->getDecl() : nullptr;
      if (shouldLogInterestingMallocDecl(CurDecl)) {
        llvm::errs() << "[CTU-MC] dispatch -> FreeMemAux (OO_Array_Delete)\n";
      }
    }
    ...
    if (CSA_CTU_DBG() && 0) {
      const clang::Decl *CurDecl =
          C.getLocationContext() ? C.getLocationContext()->getDecl() : nullptr;
      if (shouldLogInterestingMallocDecl(CurDecl)) {
        llvm::errs() << "[CTU-MC] state(after OO_Array_Delete)="
                     << (const void *)State.get() << "\n";
      }
    }
    break;
  ...
  if (CSA_CTU_DBG() && 0) {
    const clang::Decl *CurDecl =
        C.getLocationContext() ? C.getLocationContext()->getDecl() : nullptr;
    if (shouldLogInterestingMallocDecl(CurDecl)) {
      llvm::errs() << "[CTU-MC] addTransition state="
                   << (const void *)State.get() << "\n";
      llvm::errs() << "[CTU-MC] ===== checkCXXNewOrCXXDelete EXIT =====\n";
    }
  }
  C.addTransition(State);
}
...
ProgramStateRef MallocChecker::FreeMemAux(CheckerContext &C,
  ...
  // NewDelete visible-allocation-only optimization.
  // Only apply this lossy pruning to C++ new/delete families, not malloc/free.
  if (isNewDeleteVisibleAllocationOnlyEnabled(C) &&
      (Family == AF_CXXNew || Family == AF_CXXNewArray)) {
    SVal ArgVal = Call.getArgSVal(Num);

    if (!isHeapSymRegionSVal(ArgVal)) {
      dumpMallocOptSkip("FreeMemAux(CallEvent)", "not-heap-sym-region", ArgVal);
      IsKnownToBeAllocated = false;
      return State;
    }
  }
...
ProgramStateRef MallocChecker::FreeMemAux(
  ...
  // Normal free.
  if (Hold) {
    ProgramStateRef NewState = State->set<RegionState>(
  ...
  ProgramStateRef NewState = State->set<RegionState>(
      SymBase, RefState::getReleased(Family, ParentExpr));
  return NewState;
...
void MallocChecker::HandleLeak(SymbolRef Sym, ExplodedNode *N,
  ...
  AllocationFamily Family = RS->getAllocationFamily();
    ...
    if (CSA_LEAK_PATH_DBG()) {
    llvm::errs() << "\n[LEAK-PATH] ===== HandleLeak candidate =====\n";

    llvm::errs() << "[LEAK-PATH] Sym=";
    Sym->dump();
    llvm::errs() << "\n";

    llvm::errs() << "[LEAK-PATH] Family=" << (int)Family << "\n";

    if (const Stmt *AllocStmt = RS->getStmt()) {
      llvm::errs() << "[LEAK-PATH] AllocStmt class="
                   << AllocStmt->getStmtClassName()
                   << "\n";
      llvm::errs() << "[LEAK-PATH] AllocLoc=";
      dumpSourceLocForLeakPath(AllocStmt->getBeginLoc(), C.getSourceManager());
      llvm::errs() << "\n";
    }

    llvm::errs() << "[LEAK-PATH] ErrorNode=" << N << "\n";

    if (const Stmt *S = N ? N->getStmtForDiagnostics() : nullptr) {
      llvm::errs() << "[LEAK-PATH] ErrorStmt class="
                   << S->getStmtClassName()
                   << "\n";
      llvm::errs() << "[LEAK-PATH] ErrorStmtLoc=";
      dumpSourceLocForLeakPath(S->getBeginLoc(), C.getSourceManager());
      llvm::errs() << "\n";
    } else {
      llvm::errs() << "[LEAK-PATH] ErrorStmt=<null>\n";
    }

    const LocationContext *LCtx = N ? N->getLocationContext() : nullptr;

    llvm::errs() << "[LEAK-PATH] Current LCtx stack:\n";
    unsigned Depth = 0;
    for (const LocationContext *LC = LCtx; LC; LC = LC->getParent()) {
      llvm::errs() << "[LEAK-PATH]   frame[" << Depth++ << "] ";
      ...
      llvm::errs() << " lambdaFrame=" << isLambdaCallOperatorDecl(D)
                   << " forEachFrame=" << isStdForEachLikeDecl(D)
                   << "\n";
    }

    llvm::errs() << "[LEAK-PATH] CurrentStackHasLambda="
                 << locCtxStackHasLambda(LCtx)
                 << "\n";
    llvm::errs() << "[LEAK-PATH] CurrentStackHasForEach="
                 << locCtxStackHasForEach(LCtx)
                 << "\n";
    llvm::errs() << "[LEAK-PATH] ExplodedPathHasLambda="
                 << explodedPathHasLambdaFrame(N)
                 << "\n";
    llvm::errs() << "[LEAK-PATH] ExplodedPathHasForEach="
                 << explodedPathHasForEachFrame(N)
                 << "\n";

    llvm::errs() << "[LEAK-PATH] ===== End candidate =====\n";
  }
  ...
  if (custom_analyzer_fixes::isReportPathFixEnabled()) {
    R->setLeakSymbolForPathSelection(Sym);

    if (const ExplodedNode *LambdaUseN = findLambdaUseNodeForLeak(N, Sym)) {
      R->setPreferredPathNode(LambdaUseN);
      R->markInteresting(LambdaUseN->getLocationContext());

      if (CSA_LEAK_PATH_DBG()) {
        llvm::errs() << "[LEAK-PATH] set preferred path node=" << LambdaUseN
                     << "\n";

        if (const Stmt *S = LambdaUseN->getStmtForDiagnostics()) {
          llvm::errs() << "[LEAK-PATH] preferred stmt="
                       << S->getStmtClassName() << " loc=";
          S->getBeginLoc().print(llvm::errs(), C.getSourceManager());
          llvm::errs() << "\n";

          llvm::errs() << "[LEAK-PATH] preferred stmt dump:\n";
          S->dump();
        }

        const LocationContext *LC = LambdaUseN->getLocationContext();
        llvm::errs() << "[LEAK-PATH] preferred LCtx stack:\n";
        unsigned Depth = 0;
        for (const LocationContext *CurLC = LC; CurLC;
             CurLC = CurLC->getParent()) {
          llvm::errs() << "[LEAK-PATH]   frame[" << Depth++ << "] ";
          if (const auto *ND = dyn_cast_or_null<NamedDecl>(CurLC->getDecl()))
            llvm::errs() << ND->getQualifiedNameAsString();
          else if (CurLC->getDecl())
            llvm::errs() << CurLC->getDecl()->getDeclKindName();
          else
            llvm::errs() << "<null>";
          llvm::errs() << "\n";
        }
...
void MallocChecker::checkPreCall(const CallEvent &Call,
  ...
  // }
    ...
    SVal DeleteArgVal = C.getSVal(DE->getArgument());

    // NewDelete visible-allocation-only optimization.
    // Skip delete paths whose argument is not backed by HeapSymRegion.
    if (isNewDeleteVisibleAllocationOnlyEnabled(C) &&
        !isHeapSymRegionSVal(DeleteArgVal)) {
      dumpMallocOptSkip("checkPreCall(CXXDeallocatorCall)",
                        "not-heap-sym-region", DeleteArgVal);
      return;
    }

    if (!ChecksEnabled[CK_NewDeleteChecker])
      if (SymbolRef Sym = DeleteArgVal.getAsSymbol())
        checkUseAfterFree(Sym, C, DE->getArgument());
  ...
  // }
    ...
    if (const auto *DC = dyn_cast<CXXDestructorCall>(&Call)) {
    SVal ThisVal = DC->getCXXThisVal();

    if (isNewDeleteVisibleAllocationOnlyEnabled(C) &&
        !isHeapSymRegionSVal(ThisVal)) {
      dumpMallocOptSkip("checkPreCall(CXXDestructorCall)",
                        "not-heap-sym-region", ThisVal);
      return;
    }
  ...
  // }
    ...
    if (const CXXInstanceCall *CC = dyn_cast<CXXInstanceCall>(&Call)) {
    SVal ThisVal = CC->getCXXThisVal();

    if (isNewDeleteVisibleAllocationOnlyEnabled(C) &&
        !isHeapSymRegionSVal(ThisVal)) {
      dumpMallocOptSkip("checkPreCall(CXXInstanceCall-this)",
                        "not-heap-sym-region", ThisVal);
    } else {
      SymbolRef Sym = ThisVal.getAsSymbol();
      if (!Sym || checkUseAfterFree(Sym, C, CC->getCXXThisExpr()))
  ...
  for (unsigned I = 0, E = Call.getNumArgs(); I != E; ++I) {
    ...
    if (isa<Loc>(ArgSVal)) {
      if (isNewDeleteVisibleAllocationOnlyEnabled(C) &&
          !isHeapSymRegionSVal(ArgSVal)) {
        dumpMallocOptSkip("checkPreCall(arg)", "not-heap-sym-region", ArgSVal);
        continue;
      }
...
void MallocChecker::checkEscapeOnReturn(const ReturnStmt *S,
  ...
  // NewDelete visible-allocation-only optimization.
  // If enabled, ignore UAF checks for return values not backed by HeapSymRegion.
  if (Sym && isNewDeleteVisibleAllocationOnlyEnabled(C) &&
      !isHeapSymRegionSVal(RetVal)) {
    dumpMallocOptSkip("checkEscapeOnReturn", "not-heap-sym-region", RetVal);
    return;
  }
...
static const CXXDeleteExpr *getOriginCXXDeleteExpr(const CallEvent *Call) {
  if (!Call)
    return nullptr;

  return dyn_cast_or_null<CXXDeleteExpr>(Call->getOriginExpr());
}

class DeletedObjectReachableSymbolCollector final : public SymbolVisitor {
  llvm::SmallPtrSetImpl<SymbolRef> &Symbols;

public:
  explicit DeletedObjectReachableSymbolCollector(
      llvm::SmallPtrSetImpl<SymbolRef> &Symbols)
      : Symbols(Symbols) {}
...
static SymbolRef getDeletedObjectSymbolFromDeleteCall(const CallEvent *Call) {
  if (!Call || Call->getNumArgs() == 0)
    return nullptr;

  SVal DeleteArg = Call->getArgSVal(0);

  if (SymbolRef Sym = DeleteArg.getAsLocSymbol())
    return Sym;

  if (const MemRegion *R = DeleteArg.getAsRegion()) {
    R = R->StripCasts();

    if (const auto *SR = dyn_cast_or_null<SymbolicRegion>(R->getBaseRegion()))
      return SR->getSymbol();
  }
...
static const CXXRecordDecl *getDeletedCXXRecordDecl(const CallEvent *Call) {
  if (!Call)
    return nullptr;

  const auto *DE = dyn_cast_or_null<CXXDeleteExpr>(Call->getOriginExpr());
  if (!DE)
    return nullptr;

  QualType Ty = DE->getDestroyedType();

  // If getDestroyedType() is unavailable or returns a pointer type in your
  // local version, fall back to the delete argument's pointee type.
  if (Ty.isNull()) {
    Ty = DE->getArgument()->getType();
    if (Ty->isPointerType())
...
static bool shouldSkipEscapeFromDeletedObjectReachableField(
    const CallEvent *Call, ProgramStateRef State, SymbolRef Sym) {
  const CXXRecordDecl *DeletedRD = getDeletedCXXRecordDecl(Call);
  if (!DeletedRD)
    return false;

  // Only apply this rule to record objects that may contain raw pointer fields.
  // For delete p where p is int*, there is no CXXRecordDecl and this returns
  // false.
  if (!CSA_HOLDER_RecordHasPointerField(DeletedRD))
    return false;

  SymbolRef DeletedObjSym = getDeletedObjectSymbolFromDeleteCall(Call);
  if (!DeletedObjSym)
    return false;

  // Never suppress escape handling for the object being deleted itself.
  if (Sym == DeletedObjSym)
    return false;
  ...
  if (!checkIfNewOrNewArrayFamily(RS))
    return false;

  if (CSA_HOLDER_DBG()) {
    llvm::errs()
        << "[HOLDER-DELETE-SKIP] SKIP non-deleted CXXNew symbol during delete\n";

    llvm::errs() << "[HOLDER-DELETE-SKIP] deleted record="
                 << DeletedRD->getQualifiedNameAsString()
                 << "\n";

    llvm::errs() << "[HOLDER-DELETE-SKIP] deleted object sym=";
    DeletedObjSym->dump();
    llvm::errs() << "\n";

    llvm::errs() << "[HOLDER-DELETE-SKIP] candidate sym=";
    Sym->dump();
    llvm::errs() << "\n";
  }
...
static bool CSA_HOLDER_HasTrackedCXXNewInEscaped(
    ProgramStateRef State, const InvalidatedSymbols &Escaped) {
  if (!CSA_HOLDER_DBG())
    return false;
...
static void CSA_HOLDER_DumpCallBrief(const CallEvent *Call) {
  if (!Call) {
    llvm::errs() << "[HOLDER-AUX] Call=<null>\n";
    return;
  }

  if (const Decl *D = Call->getDecl()) {
    llvm::errs() << "[HOLDER-AUX] call decl kind="
                 << D->getDeclKindName()
                 << "\n";
    D->dump();
  } else {
    llvm::errs() << "[HOLDER-AUX] call decl=<null>\n";
  }

  if (const auto *IC = dyn_cast<CXXInstanceCall>(Call)) {
    llvm::errs() << "[HOLDER-AUX] this=";
    IC->getCXXThisVal().dump();
    llvm::errs() << "\n";
  }

  if (const Expr *Origin = Call->getOriginExpr()) {
    llvm::errs() << "[HOLDER-AUX] origin expr class="
                 << Origin->getStmtClassName()
                 << "\n";
  }
}
...
ProgramStateRef MallocChecker::checkPointerEscapeAux(
    ...
    bool IsConstPointerEscape) const {
  const bool HolderTrace =
      CSA_HOLDER_HasTrackedCXXNewInEscaped(State, Escaped);

  if (HolderTrace) {
    llvm::errs() << "[HOLDER-AUX] enter checkPointerEscapeAux\n";
    llvm::errs() << "[HOLDER-AUX] Kind=" << Kind
                 << " IsConstPointerEscape=" << IsConstPointerEscape
                 << " Escaped.size=" << Escaped.size()
                 << "\n";

    CSA_HOLDER_DumpCallBrief(Call);

    unsigned I = 0;
    for (SymbolRef Sym : Escaped) {
      llvm::errs() << "[HOLDER-AUX] escaped[" << I++ << "]=";
      Sym->dump();
      llvm::errs() << "\n";

      const RefState *RS = State->get<RegionState>(Sym);
      if (!RS) {
        llvm::errs() << "[HOLDER-AUX]   RegionState=<null>\n";
        continue;
      }

      llvm::errs() << "[HOLDER-AUX]   allocated=" << RS->isAllocated()
                   << " size0=" << RS->isAllocatedOfSizeZero()
                   << " released=" << RS->isReleased()
                   << " relinquished=" << RS->isRelinquished()
                   << " escaped=" << RS->isEscaped()
      ...
      if (const Stmt *S = RS->getStmt()) {
        llvm::errs() << "[HOLDER-AUX]   alloc stmt class="
                     << S->getStmtClassName()
                     << "\n";
      }
    }
  ...
  if (Kind == PSK_DirectEscapeOnCall &&
      ...
      !EscapingSymbol) {
    if (HolderTrace) {
      llvm::errs() << "[HOLDER-AUX] early return before escape\n";
    }
    return State;
  }

  const bool EnableStructSubobjectFix =
      custom_analyzer_fixes::isStructSubobjectInvalidationFixEnabled();
  ...
  for (InvalidatedSymbols::const_iterator I = Escaped.begin(),
    ...
    if (EscapingSymbol && EscapingSymbol != Sym) {
      if (HolderTrace) {
        llvm::errs() << "[HOLDER-AUX] skip due to EscapingSymbol filter Sym=";
        Sym->dump();
        llvm::errs() << "\n";
      }
      continue;
    ...
    if (EnableStructSubobjectFix &&
        shouldSkipLambdaFieldNonReachableEscape(Call, State, Sym))
      continue;

    if (EnableStructSubobjectFix &&
        shouldSkipNonFieldReachableEscapeFromFieldDtor(Call, State, Sym)) {
      if (CSA_HOLDER_DBG()) {
        llvm::errs()
            << "[HOLDER-AUX] SKIP non-field-reachable escape from field dtor Sym=";
        Sym->dump();
        llvm::errs() << "\n";
      }
      continue;
    ...
    if (EnableStructSubobjectFix &&
        shouldSkipEscapeFromDeletedObjectReachableField(Call, State, Sym))
      continue;
...
//   bool InLambda = false;
//   for (const LocationContext *CurLC = LC; CurLC; CurLC = CurLC->getParent()) {
//     const auto *MD = dyn_cast_or_null<CXXMethodDecl>(CurLC->getDecl());
//     if (MD && MD->getParent() && MD->getParent()->isLambda()) {
//       InLambda = true;
//       break;
//     }
...
//     bool MentionsLeakedSym = false;

//     if (const Expr *E = dyn_cast<Expr>(S)) {
//       SVal V = State->getSVal(E, LC);
//       if (V.getAsSymbol() == Sym || V.getAsLocSymbol() == Sym)
//         MentionsLeakedSym = true;
//     }

//     // Also catch simple `(void)p` / DeclRefExpr-like statements where the
//     // statement itself may not evaluate exactly to the allocation symbol.
//     if (!MentionsLeakedSym) {
//       for (const Stmt *Child : S->children()) {
//         if (!Child)
//           continue;

//         if (const Expr *CE = dyn_cast<Expr>(Child)) {
//           SVal V = State->getSVal(CE, LC);
//           if (V.getAsSymbol() == Sym || V.getAsLocSymbol() == Sym) {
//             MentionsLeakedSym = true;
//             break;
//           }
...
//           PathDiagnosticLocation::createBegin(S, BRC.getSourceManager(), LC);

//       return std::make_shared<PathDiagnosticEventPiece>(
//           Pos, "Memory is still allocated on this callback path", true);
//     }
//   }
// }
...
//   if (isInLambdaContext(LCtx) &&
//       stmtTreeCanReachSymbol(S, state, LCtx, Sym)) {
//     if (CSA_LEAK_PATH_DBG()) {
//       llvm::errs() << "[LEAK-PATH-VISITOR] emit lambda leak note\n";
//       llvm::errs() << "[LEAK-PATH-VISITOR] stmt class="
//                    << S->getStmtClassName()
//                    << "\n";
//       llvm::errs() << "[LEAK-PATH-VISITOR] sym=";
//       Sym->dump();
//       llvm::errs() << "\n";
//     }

//     PathDiagnosticLocation Pos =
//         PathDiagnosticLocation::createBegin(S, BRC.getSourceManager(), LCtx);

//     return std::make_shared<PathDiagnosticEventPiece>(
//         Pos, "Memory remains allocated on this callback path", true);
//   }
// }
  ...
  if (!S && (!RSCurr || RSCurr->getAllocationFamily() != AF_InnerBuffer)) {
    if (CSA_CTU_DBG() && 0) {
      llvm::errs()
          << "[CTU-MBV] return nullptr: no stmt and not inner buffer\n";
    }
    return nullptr;
  }
  ...
  if (CSA_CTU_DBG() && 0) {
    llvm::errs() << "[CTU-MBV] return piece ptr=" << (const void *)P.get()
                 << "\n";
  }
