...
static llvm::StringRef astwriteTplKindToString(FunctionDecl::TemplatedKind K) {
  switch (K) {
...
static bool astwriteIsInterestingFunction(const FunctionDecl *FD) {
  if (!FD)
    return false;
...
static void astwriteDumpLoc(llvm::StringRef Tag, const Decl *D) {
  if (!CSA_CTU_DBG())
    return;
  ...
  llvm::errs() << " isInMainFile=" << SM.isInMainFile(SM.getExpansionLoc(Loc))
               << "\n";
}

static void astwriteDumpFunctionState(llvm::StringRef Tag,
  ...
  llvm::errs() << " FD=" << (const void *)FD
               ...
               << astwriteTplKindToString(FD->getTemplatedKind())
  ...
  astwriteDumpLoc("[ASTWRITE-TPL]   FD", FD);

  if (Def && Def->getBody()) {
    llvm::errs() << "[ASTWRITE-TPL]   Def body dump begin\n";
    Def->getBody()->dump();
    llvm::errs() << "[ASTWRITE-TPL]   Def body dump end\n";
  } else if (FD->getBody()) {
    llvm::errs() << "[ASTWRITE-TPL]   FD direct body dump begin\n";
    FD->getBody()->dump();
    llvm::errs() << "[ASTWRITE-TPL]   FD direct body dump end\n";
  } else {
    llvm::errs() << "[ASTWRITE-TPL]   body=<null>\n";
  }
}

static void astwriteDumpSpecializationInfo(const FunctionDecl *D) {
  if (!CSA_CTU_DBG())
    return;
  ...
  const FunctionTemplateSpecializationInfo *FTSInfo =
      D->getTemplateSpecializationInfo();

  if (!FTSInfo) {
    llvm::errs() << "[ASTWRITE-TPL]   specialization info=<null>\n";
    return;
  }

  llvm::errs() << "[ASTWRITE-TPL]   specialization info="
               << (const void *)FTSInfo << "\n";
  llvm::errs() << "[ASTWRITE-TPL]   specialization kind="
               << FTSInfo->getTemplateSpecializationKind() << "\n";

  if (FunctionTemplateDecl *Template = FTSInfo->getTemplate()) {
    llvm::errs() << "[ASTWRITE-TPL]   info template=" << (const void *)Template
                 << " name=" << Template->getNameAsString() << "\n";
  } else {
    llvm::errs() << "[ASTWRITE-TPL]   info template=<null>\n";
  }

  SourceLocation POI = FTSInfo->getPointOfInstantiation();
  if (POI.isValid()) {
    const SourceManager &SM = D->getASTContext().getSourceManager();
    PresumedLoc PL = SM.getPresumedLoc(SM.getExpansionLoc(POI));

    if (PL.isValid()) {
      llvm::errs() << "[ASTWRITE-TPL]   point of instantiation="
                   << PL.getFilename() << ":" << PL.getLine() << ":"
                   << PL.getColumn() << "\n";
    } else {
      llvm::errs()
          << "[ASTWRITE-TPL]   point of instantiation=<invalid presumed loc>\n";
    }
  } else {
    llvm::errs() << "[ASTWRITE-TPL]   point of instantiation=<invalid>\n";
  }

  if (const TemplateArgumentList *Args = D->getTemplateSpecializationArgs()) {
    llvm::errs() << "[ASTWRITE-TPL]   template args size=" << Args->size()
                 << "\n";

    for (unsigned I = 0; I < Args->size(); ++I) {
      llvm::errs() << "[ASTWRITE-TPL]     arg[" << I << "] ";
      Args->get(I).print(D->getASTContext().getPrintingPolicy(), llvm::errs(),
                         true);
      llvm::errs() << "\n";
    }
  } else {
    llvm::errs() << "[ASTWRITE-TPL]   template args=<null>\n";
  }
}
...
void ASTDeclWriter::VisitFunctionDecl(FunctionDecl *D) {
  ...
  if (CSA_CTU_DBG() && astwriteIsInterestingFunction(D)) {
    llvm::errs() << "\n[ASTWRITE-TPL] ===== VisitFunctionDecl ENTER =====\n";
    astwriteDumpFunctionState("[ASTWRITE-TPL] enter:", D);
    astwriteDumpSpecializationInfo(D);
  }
  ...
  if (CSA_CTU_DBG() && astwriteIsInterestingFunction(D)) {
    llvm::errs()
        << "[ASTWRITE-TPL] after VisitRedeclarable/VisitDeclaratorDecl\n";
    astwriteDumpFunctionState("[ASTWRITE-TPL] state:", D);
    astwriteDumpSpecializationInfo(D);
  }
  ...
  if (CSA_CTU_DBG() && astwriteIsInterestingFunction(D)) {
    llvm::errs() << "[ASTWRITE-TPL] after basic FunctionDecl fields\n";
    llvm::errs() << "[ASTWRITE-TPL]   storageClass="
                 << static_cast<int>(D->getStorageClass())
                 << " inlineSpecified=" << D->isInlineSpecified()
                 << " isInlined=" << D->isInlined() << " isPure=" << D->isPure()
                 << " skippedBody=" << D->hasSkippedBody()
                 << " lateTemplateParsed=" << D->isLateTemplateParsed()
                 << " linkage=" << D->getLinkageInternal()
                 << " ODRHash=" << D->getODRHash() << "\n";
    astwriteDumpFunctionState("[ASTWRITE-TPL] state:", D);
  }
  ...
  FunctionDecl::TemplatedKind TK = D->getTemplatedKind();

  if (CSA_CTU_DBG() && astwriteIsInterestingFunction(D)) {
    llvm::errs() << "[ASTWRITE-TPL] before write templated kind"
                 << " kind=" << astwriteTplKindToString(TK)
                 << " raw=" << static_cast<unsigned>(TK) << "\n";
    astwriteDumpFunctionState("[ASTWRITE-TPL] state:", D);
    astwriteDumpSpecializationInfo(D);
  }
  ...
  case FunctionDecl::TK_FunctionTemplate:
    if (CSA_CTU_DBG() && astwriteIsInterestingFunction(D)) {
      llvm::errs() << "[ASTWRITE-TPL] case TK_FunctionTemplate\n";
      llvm::errs() << "[ASTWRITE-TPL]   describedTemplate="
                   << (const void *)D->getDescribedFunctionTemplate();
      if (D->getDescribedFunctionTemplate())
        llvm::errs() << " name="
                     << D->getDescribedFunctionTemplate()->getNameAsString();
      llvm::errs() << "\n";
    }
  ...
  case FunctionDecl::TK_FunctionTemplateSpecialization: {
    ...
    if (CSA_CTU_DBG() && astwriteIsInterestingFunction(D)) {
      llvm::errs() << "\n[ASTWRITE-TPL] ----- "
                      "TK_FunctionTemplateSpecialization begin -----\n";
      astwriteDumpFunctionState("[ASTWRITE-TPL] specialization FD:", D);
      astwriteDumpSpecializationInfo(D);

      llvm::errs() << "[ASTWRITE-TPL]   FTSInfo=" << (const void *)FTSInfo
                   << "\n";
      if (FTSInfo) {
        llvm::errs() << "[ASTWRITE-TPL]   FTSInfo->getTemplate()="
                     << (const void *)FTSInfo->getTemplate();
        if (FTSInfo->getTemplate())
          llvm::errs() << " name=" << FTSInfo->getTemplate()->getNameAsString();
        llvm::errs() << "\n";

        llvm::errs() << "[ASTWRITE-TPL]   FTSInfo kind="
                     << FTSInfo->getTemplateSpecializationKind() << "\n";
      }
    }
    ...
    if (CSA_CTU_DBG() && astwriteIsInterestingFunction(D)) {
      llvm::errs() << "[ASTWRITE-TPL] after RegisterTemplateSpecialization\n";
      astwriteDumpFunctionState("[ASTWRITE-TPL] state:", D);
    }
    ...
    if (CSA_CTU_DBG() && astwriteIsInterestingFunction(D)) {
      llvm::errs() << "[ASTWRITE-TPL] wrote template argument list\n";
      if (const TemplateArgumentList *Args =
              D->getTemplateSpecializationArgs()) {
        llvm::errs() << "[ASTWRITE-TPL]   args size=" << Args->size() << "\n";
        for (unsigned I = 0; I < Args->size(); ++I) {
          llvm::errs() << "[ASTWRITE-TPL]     arg[" << I << "] ";
          Args->get(I).print(D->getASTContext().getPrintingPolicy(),
                             llvm::errs(), true);
          llvm::errs() << "\n";
        }
      }
    ...
    if (CSA_CTU_DBG() && astwriteIsInterestingFunction(D)) {
      SourceLocation POI = FTSInfo->getPointOfInstantiation();
      llvm::errs() << "[ASTWRITE-TPL] wrote POI=";
    ...
    if (CSA_CTU_DBG() && astwriteIsInterestingFunction(D)) {
      llvm::errs() << "[ASTWRITE-TPL] D->isCanonicalDecl()="
                   << D->isCanonicalDecl() << "\n";
    }
    ...
    if (D->isCanonicalDecl()) {
      ...
      if (CSA_CTU_DBG() && astwriteIsInterestingFunction(D)) {
        llvm::errs()
            << "[ASTWRITE-TPL] write canonical template ref="
            << (const void *)FTSInfo->getTemplate()->getCanonicalDecl();
        if (FTSInfo->getTemplate())
          llvm::errs() << " name=" << FTSInfo->getTemplate()->getNameAsString();
        llvm::errs() << "\n";
      }
    ...
    if (CSA_CTU_DBG() && astwriteIsInterestingFunction(D)) {
      llvm::errs() << "[ASTWRITE-TPL] ----- TK_FunctionTemplateSpecialization "
                      "end -----\n";
      astwriteDumpFunctionState("[ASTWRITE-TPL] final state in case:", D);
      astwriteDumpSpecializationInfo(D);
    }
  ...
  if (CSA_CTU_DBG() && astwriteIsInterestingFunction(D)) {
    llvm::errs() << "[ASTWRITE-TPL] after params / VisitFunctionDecl END\n";
    astwriteDumpFunctionState("[ASTWRITE-TPL] end:", D);
    astwriteDumpSpecializationInfo(D);
  }
}
