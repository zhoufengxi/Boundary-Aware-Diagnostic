...
//===----------------------------------------------------------------------===//
static bool CSA_CTU_DBG() { return std::getenv("CSA_CTU_DBG") != nullptr && 0; }

static llvm::StringRef astreadTplKindToString(FunctionDecl::TemplatedKind K) {
  switch (K) {
...
static bool astreadIsInterestingFunction(const FunctionDecl *FD) {
  if (!FD)
    return false;

  if (FD->getQualifiedNameAsString() == "tfunc")
    return true;
...
static void astreadDumpLoc(llvm::StringRef Tag, const Decl *D) {
  if (!CSA_CTU_DBG())
    return;
  ...
  PresumedLoc PL = SM.getPresumedLoc(SM.getExpansionLoc(Loc));
...
static void astreadDumpFunctionState(llvm::StringRef Tag,
  ...
  llvm::errs() << " FD=" << (const void *)FD
               << " qname=" << FD->getQualifiedNameAsString()
               << " type=" << FD->getType().getAsString() << " templatedKind="
               << astreadTplKindToString(FD->getTemplatedKind())
               << " hasBody=" << HasBody << " Def=" << (const void *)Def
               << " getDefinition=" << (const void *)FD->getDefinition()
               << " getBody=" << (const void *)FD->getBody()
               << " doesThisHaveBody=" << FD->doesThisDeclarationHaveABody()
  ...
  astreadDumpLoc("[ASTREAD-TPL]   FD", FD);

  if (Def && Def->getBody()) {
    llvm::errs() << "[ASTREAD-TPL]   Def body dump begin\n";
    Def->getBody()->dump();
    llvm::errs() << "[ASTREAD-TPL]   Def body dump end\n";
  } else if (FD->getBody()) {
    llvm::errs() << "[ASTREAD-TPL]   FD direct body dump begin\n";
    FD->getBody()->dump();
    llvm::errs() << "[ASTREAD-TPL]   FD direct body dump end\n";
  } else {
    llvm::errs() << "[ASTREAD-TPL]   body=<null>\n";
  }
}

static void astreadDumpSpecializationInfo(const FunctionDecl *FD) {
  if (!CSA_CTU_DBG())
    return;
  ...
  if (!Info) {
    llvm::errs() << "[ASTREAD-TPL]   specialization info=<null>\n";
    return;
  }

  llvm::errs() << "[ASTREAD-TPL]   specialization info=" << (const void *)Info
               << "\n";
  llvm::errs() << "[ASTREAD-TPL]   specialization kind="
               << Info->getTemplateSpecializationKind() << "\n";

  if (FunctionTemplateDecl *Template = Info->getTemplate()) {
    llvm::errs() << "[ASTREAD-TPL]   info template=" << (const void *)Template
                 << " name=" << Template->getNameAsString() << "\n";
  } else {
    llvm::errs() << "[ASTREAD-TPL]   info template=<null>\n";
  }
  ...
  if (POI.isValid()) {
    ...
    if (PL.isValid()) {
      llvm::errs() << "[ASTREAD-TPL]   point of instantiation="
                   << PL.getFilename() << ":" << PL.getLine() << ":"
                   << PL.getColumn() << "\n";
    } else {
      llvm::errs()
          << "[ASTREAD-TPL]   point of instantiation=<invalid presumed loc>\n";
    }
  } else {
    llvm::errs() << "[ASTREAD-TPL]   point of instantiation=<invalid>\n";
  }

  if (const TemplateArgumentList *Args = FD->getTemplateSpecializationArgs()) {
    llvm::errs() << "[ASTREAD-TPL]   template args size=" << Args->size()
                 << "\n";

    for (unsigned I = 0; I < Args->size(); ++I) {
      llvm::errs() << "[ASTREAD-TPL]     arg[" << I << "] ";
      Args->get(I).print(FD->getASTContext().getPrintingPolicy(), llvm::errs(),
                         true);
      llvm::errs() << "\n";
    }
  } else {
    llvm::errs() << "[ASTREAD-TPL]   template args=<null>\n";
  }
}
...
void ASTDeclReader::Visit(Decl *D) {
  if (CSA_CTU_DBG()) {
    llvm::errs() << "\n[ASTREAD-VISIT] ===== Visit(Decl*) ENTER =====\n";
    llvm::errs() << "[ASTREAD-VISIT] D=" << (const void *)D
  ...
  if (CSA_CTU_DBG()) {
    llvm::errs() << "[ASTREAD-VISIT] after DeclVisitor\n";
    llvm::errs() << "[ASTREAD-VISIT] D=" << (const void *)D
                 << " kind=" << D->getDeclKindName()
                 << " canonical=" << (const void *)D->getCanonicalDecl()
                 << " IsDeclMarkedUsed=" << IsDeclMarkedUsed << "\n";

    if (auto *FD = dyn_cast<FunctionDecl>(D)) {
      if (astreadIsInterestingFunction(FD)) {
        llvm::errs()
            << "[ASTREAD-VISIT] interesting FunctionDecl after DeclVisitor\n";
        astreadDumpFunctionState("[ASTREAD-VISIT]   FD:", FD);
        astreadDumpSpecializationInfo(FD);
      }
    }
  }
  ...
  if (CSA_CTU_DBG()) {
    if (auto *FD = dyn_cast<FunctionDecl>(D)) {
      if (astreadIsInterestingFunction(FD)) {
        llvm::errs() << "[ASTREAD-VISIT] after canonical Used update\n";
        llvm::errs() << "[ASTREAD-VISIT]   canonical="
                     << (const void *)D->getCanonicalDecl()
                     << " canonicalUsed=" << D->getCanonicalDecl()->Used
                     << "\n";
      }
    }
  }
    ...
    if (auto *TInfo = DD->getTypeSourceInfo()) {
      if (CSA_CTU_DBG()) {
        if (auto *FD = dyn_cast<FunctionDecl>(D)) {
          if (astreadIsInterestingFunction(FD)) {
            llvm::errs()
                << "[ASTREAD-VISIT] before read TypeLoc for DeclaratorDecl\n";
            astreadDumpFunctionState("[ASTREAD-VISIT]   FD:", FD);
          }
        }
      }
      ...
      if (CSA_CTU_DBG()) {
        if (auto *FD = dyn_cast<FunctionDecl>(D)) {
          if (astreadIsInterestingFunction(FD)) {
            llvm::errs()
                << "[ASTREAD-VISIT] after read TypeLoc for DeclaratorDecl\n";
            astreadDumpFunctionState("[ASTREAD-VISIT]   FD:", FD);
          }
        }
      }
  ...
  if (auto *TD = dyn_cast<TypeDecl>(D)) {
    ...
    if (CSA_CTU_DBG() && astreadIsInterestingFunction(FD)) {
      llvm::errs() << "\n[ASTREAD-TPL-BODY] ===== FunctionDecl body handling "
                      "begin =====\n";
      astreadDumpFunctionState("[ASTREAD-TPL-BODY] before body flag:", FD);
      astreadDumpSpecializationInfo(FD);
    }
    ...
    bool HasBodyRecord = Record.readInt();

    if (CSA_CTU_DBG() && astreadIsInterestingFunction(FD)) {
      llvm::errs() << "[ASTREAD-TPL-BODY] body record flag=" << HasBodyRecord
                   << "\n";
      llvm::errs() << "[ASTREAD-TPL-BODY] before ReadFunctionDefinition check: "
                   << "FD->getBody=" << (const void *)FD->getBody()
                   << " doesThisHaveBody=" << FD->doesThisDeclarationHaveABody()
                   << " hasSkippedBody=" << FD->hasSkippedBody() << "\n";
    }

    if (HasBodyRecord) {
      if (CSA_CTU_DBG() && astreadIsInterestingFunction(FD)) {
        llvm::errs() << "[ASTREAD-TPL-BODY] about to ReadFunctionDefinition\n";
        astreadDumpFunctionState("[ASTREAD-TPL-BODY] before read:", FD);
      }

      ReadFunctionDefinition(FD);

      if (CSA_CTU_DBG() && astreadIsInterestingFunction(FD)) {
        llvm::errs() << "[ASTREAD-TPL-BODY] after ReadFunctionDefinition\n";
        astreadDumpFunctionState("[ASTREAD-TPL-BODY] after read:", FD);
        astreadDumpSpecializationInfo(FD);
      }
    } else {
      if (CSA_CTU_DBG() && astreadIsInterestingFunction(FD)) {
        llvm::errs()
            << "[ASTREAD-TPL-BODY] skip ReadFunctionDefinition because "
               "body record flag is 0\n";
      }
    }

    if (CSA_CTU_DBG() && astreadIsInterestingFunction(FD)) {
      llvm::errs()
          << "[ASTREAD-TPL-BODY] ===== FunctionDecl body handling end =====\n";
      astreadDumpFunctionState("[ASTREAD-TPL-BODY] final:", FD);
    }
  }
  ...
  if (CSA_CTU_DBG()) {
    if (auto *FD = dyn_cast<FunctionDecl>(D)) {
      if (astreadIsInterestingFunction(FD)) {
        llvm::errs()
            << "[ASTREAD-VISIT] ===== Visit(Decl*) EXIT interesting FD =====\n";
        astreadDumpFunctionState("[ASTREAD-VISIT] exit FD:", FD);
        astreadDumpSpecializationInfo(FD);
      }
    }
  }
...
void ASTDeclReader::VisitDecl(Decl *D) {
  ...
  if (CSA_CTU_DBG()) {
    llvm::errs() << "\n[ASTREAD-VISITDECL] ===== VisitDecl ENTER =====\n";
    llvm::errs() << "[ASTREAD-VISITDECL] D=" << (const void *)D
                 << " kind=" << D->getDeclKindName()
  ...
  if (D->isTemplateParameter() || D->isTemplateParameterPack() ||
    ...
    if (CSA_CTU_DBG()) {
      llvm::errs() << "[ASTREAD-VISITDECL] template/parm context IDs: "
                   << "SemaDCID=" << SemaDCIDForTemplateParmDecl
                   << " LexicalDCID=" << LexicalDCIDForTemplateParmDecl << "\n";
    }
    ...
    if (CSA_CTU_DBG()) {
      llvm::errs() << "[ASTREAD-VISITDECL] set template/parm DeclContext to TU="
                   << (const void *)Reader.getContext().getTranslationUnitDecl()
                   << "\n";
    }
  } else {
    ...
    if (CSA_CTU_DBG()) {
      llvm::errs() << "[ASTREAD-VISITDECL] read contexts: "
                   << "SemaDC=" << (const void *)SemaDC
                   << " LexicalDC=" << (const void *)LexicalDC << "\n";
    }
    ...
    if (CSA_CTU_DBG()) {
      llvm::errs() << "[ASTREAD-VISITDECL] MergedSemaDC="
                   << (const void *)MergedSemaDC << " finalSemaDC="
                   << (const void *)(MergedSemaDC ? MergedSemaDC : SemaDC)
                   << " finalLexicalDC=" << (const void *)LexicalDC << "\n";
    }
  ...
  if (CSA_CTU_DBG()) {
    llvm::errs() << "[ASTREAD-VISITDECL] after setLocation\n";
    astreadDumpLoc("[ASTREAD-VISITDECL]   D", D);
    llvm::errs() << "[ASTREAD-VISITDECL]   DeclContext="
                 << (const void *)D->getDeclContext() << " LexicalDeclContext="
  ...
  if (CSA_CTU_DBG()) {
    llvm::errs() << "[ASTREAD-VISITDECL] InvalidDecl=" << D->InvalidDecl
                 << "\n";
  }
  ...
  if (CSA_CTU_DBG()) {
    llvm::errs() << "[ASTREAD-VISITDECL] hasAttrs=" << HasAttrs << "\n";
  }
  ...
  if (HasAttrs) {
    ...
    if (CSA_CTU_DBG()) {
      llvm::errs() << "[ASTREAD-VISITDECL] read attrs count=" << Attrs.size()
                   << "\n";
    }
  ...
  D->setImplicit(IsImplicit);

  bool Used = Record.readInt();
  D->Used = Used;
  IsDeclMarkedUsed |= D->Used;

  bool Referenced = Record.readInt();
  D->setReferenced(Referenced);

  bool TopLevelInObjCContainer = Record.readInt();
  D->setTopLevelDeclInObjCContainer(TopLevelInObjCContainer);
  ...
  if (CSA_CTU_DBG()) {
    llvm::errs() << "[ASTREAD-VISITDECL] flags: "
                 << "implicit=" << IsImplicit << " used=" << Used
                 << " referenced=" << Referenced
                 << " topLevelObjC=" << TopLevelInObjCContainer
                 << " access=" << AS << " FromASTFile=" << D->FromASTFile
                 << " moduleOwnership=" << (unsigned)ModuleOwnership
  ...
  if (unsigned SubmoduleID = readSubmoduleID()) {
    if (CSA_CTU_DBG()) {
      llvm::errs() << "[ASTREAD-VISITDECL] SubmoduleID=" << SubmoduleID << "\n";
    }
    ...
    if (ModulePrivate) {
      if (CSA_CTU_DBG()) {
        llvm::errs()
            << "[ASTREAD-VISITDECL] module private; no visibility update\n";
      }
    } else if (Reader.getContext().getLangOpts().ModulesLocalVisibility) {
      if (CSA_CTU_DBG()) {
        llvm::errs() << "[ASTREAD-VISITDECL] ModulesLocalVisibility enabled\n";
      }
    } else if (Module *Owner = Reader.getSubmodule(SubmoduleID)) {
      if (CSA_CTU_DBG()) {
        llvm::errs() << "[ASTREAD-VISITDECL] Owner module=" << Owner->Name
                     << " visibility=" << Owner->NameVisibility << "\n";
      }
    ...
    if (CSA_CTU_DBG()) {
      llvm::errs() << "[ASTREAD-VISITDECL] no SubmoduleID but ModulePrivate; "
                      "set ModulePrivate ownership\n";
    }
    ...
    if (CSA_CTU_DBG()) {
      llvm::errs()
          << "[ASTREAD-VISITDECL] no SubmoduleID and not ModulePrivate\n";
    }
  }

  if (CSA_CTU_DBG()) {
    llvm::errs() << "[ASTREAD-VISITDECL] ===== VisitDecl EXIT =====\n";
    llvm::errs() << "[ASTREAD-VISITDECL] D=" << (const void *)D
                 << " kind=" << D->getDeclKindName()
                 << " DeclContext=" << (const void *)D->getDeclContext()
                 << " LexicalDeclContext="
                 << (const void *)D->getLexicalDeclContext()
                 << " FromASTFile=" << D->FromASTFile << "\n";
  }
}
...
void ASTDeclReader::VisitFunctionDecl(FunctionDecl *FD) {
  if (CSA_CTU_DBG()) {
    llvm::errs() << "\n[ASTREAD-TPL] ===== VisitFunctionDecl ENTER =====\n";
    llvm::errs() << "[ASTREAD-TPL] FD=" << (const void *)FD << "\n";
  }
  ...
  if (CSA_CTU_DBG()) {
    llvm::errs() << "[ASTREAD-TPL] after VisitDeclaratorDecl\n";
    astreadDumpFunctionState("[ASTREAD-TPL]   state:", FD);
  }
  ...
  if (CSA_CTU_DBG() && astreadIsInterestingFunction(FD)) {
    llvm::errs() << "[ASTREAD-TPL] after setType\n";
    astreadDumpFunctionState("[ASTREAD-TPL]   state:", FD);
  }
  ...
  FunctionDecl::TemplatedKind TK =
      static_cast<FunctionDecl::TemplatedKind>(Record.readInt());

  if (CSA_CTU_DBG()) {
    llvm::errs() << "[ASTREAD-TPL] before templated kind switch"
                 << " rawKind=" << static_cast<unsigned>(TK)
                 << " kind=" << astreadTplKindToString(TK) << "\n";
    astreadDumpFunctionState("[ASTREAD-TPL]   state:", FD);
  }
  ...
  case FunctionDecl::TK_FunctionTemplateSpecialization: {
    if (CSA_CTU_DBG()) {
      llvm::errs() << "\n[ASTREAD-TPL] ----- TK_FunctionTemplateSpecialization "
                      "begin -----\n";
      astreadDumpFunctionState("[ASTREAD-TPL]   before read Template:", FD);
    }
    ...
    if (CSA_CTU_DBG()) {
      llvm::errs() << "[ASTREAD-TPL]   read Template="
                   << (const void *)Template;
      if (Template)
        llvm::errs() << " name=" << Template->getNameAsString();
      llvm::errs() << " TSK=" << TSK << "\n";
    }
    ...
    if (CSA_CTU_DBG()) {
      llvm::errs() << "[ASTREAD-TPL]   read template args size="
                   << TemplArgs.size() << "\n";

      for (unsigned I = 0; I < TemplArgs.size(); ++I) {
        llvm::errs() << "[ASTREAD-TPL]     arg[" << I << "] ";
        TemplArgs[I].print(Reader.getContext().getPrintingPolicy(),
                           llvm::errs(), true);
        llvm::errs() << "\n";
      }
    }
    ...
    SourceLocation POI = readSourceLocation();
    if (CSA_CTU_DBG()) {
      llvm::errs() << "[ASTREAD-TPL]   read POI=";

      if (POI.isValid()) {
        const SourceManager &SM = Reader.getContext().getSourceManager();
        PresumedLoc PL = SM.getPresumedLoc(SM.getExpansionLoc(POI));

        if (PL.isValid()) {
          llvm::errs() << PL.getFilename() << ":" << PL.getLine() << ":"
                       << PL.getColumn();
        } else {
    ...
    if (CSA_CTU_DBG()) {
      llvm::errs() << "[ASTREAD-TPL]   after Create FTInfo\n";
      astreadDumpFunctionState("[ASTREAD-TPL]   state:", FD);
      astreadDumpSpecializationInfo(FD);
    }
    if (CSA_CTU_DBG()) {
      llvm::errs() << "[ASTREAD-TPL]   FD->isCanonicalDecl()="
                   << FD->isCanonicalDecl() << "\n";
    }
    ...
    if (FD->isCanonicalDecl()) { // if canonical add to template's set.
      ...
      if (CSA_CTU_DBG()) {
        llvm::errs() << "[ASTREAD-TPL]   CanonTemplate="
                     << (const void *)CanonTemplate;
        if (CanonTemplate)
          llvm::errs() << " name=" << CanonTemplate->getNameAsString();
        llvm::errs() << "\n";
      }
      ...
      if (CSA_CTU_DBG()) {
        llvm::errs() << "[ASTREAD-TPL]   FindNodeOrInsertPos ExistingInfo="
                     << (const void *)ExistingInfo << " InsertPos=" << InsertPos
                     << "\n";
      }
      if (InsertPos) {
        if (CSA_CTU_DBG()) {
          llvm::errs() << "[ASTREAD-TPL]   InsertNode current FTInfo into "
        ...
        if (CSA_CTU_DBG()) {
          llvm::errs() << "[ASTREAD-TPL]   specialization already exists; "
                          "mergeRedeclarable with ExistingInfo->getFunction()="
                       << (const void *)(ExistingInfo
                                             ? ExistingInfo->getFunction()
                                             : nullptr)
                       << "\n";
        }
    ...
    if (CSA_CTU_DBG()) {
      llvm::errs() << "[ASTREAD-TPL] ----- TK_FunctionTemplateSpecialization "
                      "end -----\n";
      astreadDumpFunctionState("[ASTREAD-TPL]   final state in case:", FD);
      astreadDumpSpecializationInfo(FD);
    }
  ...
  if (CSA_CTU_DBG() && astreadIsInterestingFunction(FD)) {
    llvm::errs() << "[ASTREAD-TPL] after setParams / VisitFunctionDecl END\n";
    astreadDumpFunctionState("[ASTREAD-TPL]   end state:", FD);
    astreadDumpSpecializationInfo(FD);
  }
}
