...
static void dumpCTUUSR(llvm::StringRef Prefix,
                       const llvm::Optional<std::string> &USR) {
  llvm::errs() << Prefix;
  if (USR)
    llvm::errs() << *USR << "\n";
  else
...
static llvm::Optional<clang::ento::PathDiagnosticLocation>
...
static void promoteSharedAllocationOriginForDoubleFree(
    ...
    const llvm::Optional<clang::ento::PathDiagnosticLocation> &SharedAllocLoc) {
  if (!isDoubleFreeReport(Report))
    return;
