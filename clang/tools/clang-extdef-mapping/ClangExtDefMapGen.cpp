...
//     if (isNDPointerLikeType(F->getType()))
//       return true;
//   }
...
public:
  ...
  // If the body is available, do not use the coarse "record has pointer field"
  // heuristic. Let the callee's own summary decide whether it is relevant.
  if (MD->hasBody())
    return true;
...
// static std::string encodeNewDeleteRelevance(const FunctionDecl *FD) {
//   NewDeleteRelevanceSummary S = computeNewDeleteRelevanceSummary(FD);
//   return S.isRelevantForCTUImportPruning() ? "1" : "0";
// }
...
//                 << " entry-relevant=" << S.isRelevantForEntryPruning()
//                 << " ctu-relevant=" << S.isRelevantForCTUImportPruning()
//                 << " HasNew=" << S.HasNew
//                 << " HasDelete=" << S.HasDelete
...
//                 << " HasUnknownCallWithPointerArg="
//                 << S.HasUnknownCallWithPointerArg
//                 << " HasUnknownCallWithPointerReturn="
//                 << S.HasUnknownCallWithPointerReturn
//                 << "\n";
//   }
// }
...
static void dumpNewDeleteExtDefSummary(StringRef LookupName,
  ...
  if (CSA_CTU_VERBOSE()) {
    ...
    llvm::errs() << " ctu-relevant="
                 << S.isRelevantForCTUImportPruning();
...
void MapExtDefNamesConsumer::handleDecl(const Decl *D) {
  ...
  if (const auto *FTD = dyn_cast<FunctionTemplateDecl>(D)) {
    ...
    for (const auto *Spec : FTD->specializations()) {
      if (!Spec) {
        // if (CSA_CTU_DBG())
        //   llvm::errs() << "[EXTDEF-TPL] specialization[" << SpecIndex
        //                << "] <null>\n";
        ++SpecIndex;
        continue;
      }
  ...
  if (const auto *FD = dyn_cast<FunctionDecl>(D)) {
    ...
    if (HasBody && Def) {
      if (const Stmt *Body = Def->getBody()) {
        // if (CSA_CTU_DBG()) {
        //   llvm::errs() << "[EXTDEF-FD] body exists; about to addIfInMain\n";
        //   llvm::errs() << "[EXTDEF-FD] body begin loc will be used for "
        //                   "main-file check\n";
        //   extdefDumpFunctionBody("[EXTDEF-FD] Def", Def);
        // }
...
void MapExtDefNamesConsumer::addIfInMain(const DeclaratorDecl *DD,
  ...
  case UniqueExternalLinkage:
    if (SM.isInMainFile(Loc)) {
      ...
      // Generate NewDelete/NewDeleteLeaks relevance summary for functions only.
      // VarDecl entries are intentionally omitted from the relevance map.
      // Missing relevance entry means "unknown" and the analyzer must keep the
      // original CTU import behavior.
